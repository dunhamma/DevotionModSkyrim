...
|---|---|---|
| `Claude.md` | Project context, file map, current build status, decisions log | Architecture changes, build status advances, decision lands |
...
1. Confirm the content doesn't fit in an existing LIVING doc. Most additions belong inside `Claude.md` (decisions log, file map updates) or `PDV_MOD_SETUP.md` (tooling/architecture extensions), not as new files.
...
**No "see X.md" index docs.** A doc whose only purpose is to point at another doc is a staleness amplifier — when the target moves, the index lies. `Claude.md` is the natural navigation aggregation point.
...
Don't create a second LIVING doc that duplicates content already in another LIVING doc. If `PDV_MOD_SETUP.md` defines variable conventions, `Claude.md` references that doc rather than restating. Two parallel sources always drift.
...
Version history goes in `Claude.md` § Decisions Log and (eventually) the public mod page changelog. Not in the spell description.
...
Architectural and design decisions live in `Claude.md` § Decisions Log. One entry per decision. Format:
...
- `archive/HOUSECARL_SKILL_AUTHORING.md` — only relevant if PDV ever ships Claude Code skills (e.g., `pdv-race-design`, `pdv-papyrus-debug`). Defer until a clear pain point surfaces.
