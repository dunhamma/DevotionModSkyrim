...
Last revised: 2026-05-10 (v1.3 — Phase 0 cleanup complete; see §13)
...
| `PDV__ManagerQuest.psc` | Refactor | Strip buckets and `ProcessDawn` averaging. Becomes registry + helper API. |
| `PDV_MasterQuest.psc` | ~~Delete~~ **Done 2026-05-10** | Pre-rename ancestor. ESP record removed via xEdit; `.psc` and `.pex` deleted. |
| `PDV_ActionRouter.psc` | New | Subscribes to Story Manager event nodes; fans actions to deities. |
...
PDV_ActionRouter        (Start-Game-Enabled, persistent)
  ├─ subscribes to Story Manager event nodes (kill, quest, location, item)
  └─ on event → iterate PDV_FLST_AllDeities → call deity.OnAction(event, payload)
...
| Key | Range | Purpose |
|-----|-------|---------|
| `PDV.Piety` | 0–100 | Current piety. Drives tier. |
| `PDV.PietyToday` | -∞ to +∞ | Daily scratch. Reset at dawn. |
...
| Global | Purpose |
|--------|---------|
| `PDV_GLO_OriginRace` | 0=Nord, 1=Imperial, 2=Breton, 3=Altmer, 4=Bosmer, 5=Dunmer, 6=Khajiit, 7=Argonian, 8=Orsimer, 9=Redguard. Set once. See §12. |
...
`ManagerQuest.RefreshPatronMirrors()` reads the active patron's StorageUtil values into the three mirror globals. Called from:

- `AwardPiety(deity, ...)` — only if `deity == activePatron`. No-op otherwise.
- `RecomputeTier(deity)` — same condition.
...
1. Story Manager fires the `PDV_SME_KillNode` (which has the right conditions on hostile humanoid).
2. `PDV_ActionRouter` receives the event payload (killer, victim).
3. Router classifies: `KILLED_HOSTILE_HUMANOID_IN_COMBAT`.
4. Router iterates `PDV_FLST_AllDeities`. For each: `delta = deity.ScoreAction(eventType, killer, victim) * OriginMult`.
5. Router calls `ManagerQuest.AwardPiety(deity, delta, "kill_bandit")`.
6. Manager writes `StorageUtil.AdjustFloatValue(deity, "PDV.PietyToday", delta)`.
7. If `deity == activePatron`, manager calls `RefreshPatronMirrors()` — `PDV_GLO_ActivePiety` (and `ActiveTier` if it shifted) gets the new value so any condition-driven content downstream sees the update.
...
2. For each deity in `PDV_FLST_AllDeities`:
   - Read `PietyToday`, clamp to ±5, apply to `Piety` (0–100 absolute).
...
- Add PapyrusUtil as a master.
- Declare the three mirror globals in CK: `PDV_GLO_ActivePiety`, `PDV_GLO_ActiveTier`, `PDV_GLO_ActiveDeityIndex`.
- In `PDV__MainQuest`, add stage-10 fragment that initializes StorageUtil keys for the (currently single) Kyne deity if not set.
- Strip `CombatBucket / SocialBucket / LifestyleBucket` and `PDV_GLO_DevotionLevel` from `PDV__ManagerQuest`. Replace with thin helper API: `AwardPiety`, `GetPiety`, `RecomputeTier`, `GetTier`, `RefreshPatronMirrors`. All read/write StorageUtil; the refresh helper writes to the mirror globals.
- Wire `RefreshPatronMirrors` into `AwardPiety` and `RecomputeTier` (no-op when deity ≠ active patron).
- `ProcessDawn` becomes a single-deity consolidation loop (still functionally one deity at this stage). On tier change, refresh mirrors.
- Test: piety still ticks at dawn, debug trace still fires, save/load preserves piety, `PDV_GLO_ActiveTier` matches `StorageUtil.GetFloatValue(Kyne, "PDV.Tier")` at all times.

### Phase 2 — Deity-as-Quest

- Create `PDV_DeityBase.psc` (the contract). Compile.
- Create `PDV_Deity_Kyne.psc` extending it. Migrate the Kyne-specific scoring into `ScoreAction`.
- Create `PDV_FLST_AllDeities` and add Kyne.
- Refactor `ProcessDawn` to iterate the FormList.
- Test: identical behavior to Phase 1, but now structurally ready for deity #2.

### Phase 3 — ActionRouter + first Story Manager node

- Create `PDV_ActionRouter` quest. Subscribe to one Story Manager kill-event node.
- Wire it through to `Kyne.ScoreAction` and `ManagerQuest.AwardPiety`.
- Test: killing a wolf as a Kyne worshipper drops piety; killing a bandit raises it.
- This is the first end-to-end vertical slice. After this, the rest of the mod is deities + rubric tuning.

### Phase 4 — Origin + stance + tier transitions

- Create `PDV_Origin`. Run from MainQuest stage 10. Race-keyed (`PDV_GLO_OriginRace`).
- Implement stance taxonomy on `PDV_DeityBase` per §12.2 (replaces the earlier `GainMult_<Origin>` floats).
- Implement rivalry ledger per §12.3 (call site: `AwardPiety`, after stance check).
...
These are flagged for explicit decision before locking the §12.4 matrix into CK data. All have multiple in-world sources disagreeing:
...
|--------|------|-----------|--------------|
| `NATIVE` | 0 | 1.0× | None. The deity is part of the race's cultural pantheon. |
| `FOREIGN` | 1 | 0.5× | None. Accessible but unusual for the race. |
| `TABOO` | 2 | 0.75× | Optional NPC reaction trigger; faction reputation drift if implemented. Worship works, but it's culturally costly. |
| `HOSTILE` | 3 | 1.0× | **Triggers the rivalry ledger.** Worshipping this deity actively erodes piety on a defined rival deity (the player's "natural" deity for this domain). Lockable behind a discoverable cultural-break event. |
...
Adding §12 doesn't change the bones of v2 — it adds data and a small amount of glue. Concretely:
...
Added §12 (Race-conditional religion) and refactored §4 / §7 / §9 / §10 to reflect race-keyed origins, stance taxonomy, and the rivalry ledger.
...
Cost: one additional global, one int-array of length 10 per Deity quest, one rival list (Form[]/Float[] parallel arrays) per Deity quest, ~50 lines of Papyrus on `PDV_DeityBase`, plus the design work of populating the §12.4 default stance matrix across all deities. Build cost grows linearly with deity count and is independent per deity. Slotted into Phase 4.

Open lore items flagged in §10 contested-lore subsection — Trinimac canon status, Talos-Altmer framing, Hircine-Bosmer classification, Malacath-as-Daedra-or-Aedra, and Lorkhan/Shor/Sep/Lorkhaj equivalence vs. separation. These need resolution before the §12.4 matrix is locked into CK data, but the *architecture* doesn't depend on which way each is decided.

### v1.1 — 2026-05-09 — Hybrid storage adopted

Changed §1, §4, §6, §9 to reflect a hybrid storage model rather than StorageUtil-pure.
