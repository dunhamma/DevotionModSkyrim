...
- **Mode ceiling implementation (LOCKED):** The ceiling difference isn't implemented as a hard cap on piety — it's a lower piety-per-day accumulation rate in ProcessDawn applied before the dawn clamp. The multipliers are locked at **Stronghold ×1.00, City ×0.75, Legion/Exile ×0.60** of the base rate (`ORC_RATE_MULT_STRONGHOLD/CITY/LEGIONEXILE`). Against the locked 2.5/day base and 3.5/day ceiling (`PDV_PietyPaceBalancingTable.md`), this yields the calendars in `PDV_SignalDensityAudit.md`: Stronghold 10/20/34, City 13/27/45, Legion/Exile 17/33/57 in-game days to Observant/Faithful/Devoted. The multiplier scales the per-deity daily gain only; thresholds (25/50/85) are identical across modes.
