;/
    PDV_DaedricPath_Molag.psc
    PlayerDevotion - Molag Bal Daedric path
    Generated from PDV_DaedricPrinceRecordContracts.json.
/;

Scriptname PDV_DaedricPath_Molag extends PDV_DaedricPathBase
...
Function DebugRunLiveSenderProof(String hookId)
    ...
    StorageUtil.SetStringValue(GetDeityForm(), "PDV.Daedric.Molag.LastLiveHook", hookId)
...
Function DebugRenouncePath()
    ...
    StorageUtil.SetIntValue(GetDeityForm(), "PDV.Daedric.Molag.Renounced", 1)
    if Msg_Exit
        Msg_Exit.Show()
    endIf
...
Function TraceControlled(Int level, String traceText)
    if GetDebugLevel() >= level
        Debug.Trace("[PDV] MolagPath: " + traceText)
    endIf
EndFunction
