...
Int Property LIKES_DISLIKES_VERSION = 7 AutoReadOnly
...
Function HandleDaedricPrinceSignal(Int pathIndex, String sourceId)
    ...
    ShowDaedricMilestonePresentation(path, tierBefore, tierAfter)
    RequestPanelRefresh()
...
; remembered as a raw FormID so no quest alias or VMAD change is needed;
; legacy saves with sleep history but no declared bed get the declaration
; prompt on their next bed sleep. A decline re-prompts only after 3 days.
...
; Hist Adaptation rite: at substrate HIGH, sleeping on rooted ground (the
; declared bed or a remembered water) lets the player ask the root for ONE
; adaptation. One active slot, swap only via re-rite after a 7-day cooldown.
; The rite grants no piety; the cooldown is the gate. Returns true when the
; rite menu was shown so the dream yields that night.
...
Function ShowRedguardSectEntry(Int sectValue)
    ...
    if sectValue == REDGUARD_SECT_CROWN
        ShowRedguardNotification(PDV_Notif_Redguard_Sect_Crown_Entry, "You hold the Crown way: old inheritance kept.")
...
Function HandleHircineHuntRite(String reason)
    if PDV_HircinePath
        ...
        if multiplier > 0.0
            ShowDaedricMilestonePresentation(PDV_HircinePath, tierBefore, PDV_HircinePath.GetStoredTier())
...
Function SyncBretonDruidicForkBetrayalSpell(Bool shouldBeActive)
    Actor playerRef = Game.GetPlayer()
    if !playerRef || !PDV_SPEL_CreedLoss_Breton_DruidicForkBetrayal
        StorageUtil.SetIntValue(None, "PDV.CreedLoss.BretonDruidicForkBetrayalActive", 0)
        return
    endIf

    if shouldBeActive
        if !playerRef.HasSpell(PDV_SPEL_CreedLoss_Breton_DruidicForkBetrayal)
            playerRef.AddSpell(PDV_SPEL_CreedLoss_Breton_DruidicForkBetrayal, False)
        endIf
        StorageUtil.SetIntValue(None, "PDV.CreedLoss.BretonDruidicForkBetrayalActive", 1)
    else
        if playerRef.HasSpell(PDV_SPEL_CreedLoss_Breton_DruidicForkBetrayal)
            playerRef.RemoveSpell(PDV_SPEL_CreedLoss_Breton_DruidicForkBetrayal)
        endIf
        StorageUtil.SetIntValue(None, "PDV.CreedLoss.BretonDruidicForkBetrayalActive", 0)
    endIf
EndFunction
...
Function SyncArgonianRewards(Actor playerRef)
    ...
    Bool wantSithisT2 = sithisActive && voidRelation >= ARGONIAN_REWARD_T2_THRESHOLD
    Bool wantSithisT1 = sithisActive && !wantSithisT2 && voidRelation >= ARGONIAN_REWARD_T1_THRESHOLD
...
Function ProcessQueuedDaedricMilestonePresentation()
    ...
    endIf
    ShowDaedricMilestonePresentation(path, oldTier, targetTier)
EndFunction

Function ShowDaedricMilestonePresentation(PDV_DaedricPathBase path, Int oldTier, Int newTier)
...
Bool Function SendPrismaDaedricMilestoneToast(String princeName, String tierLabel, String flavorText, String boonText, String priceText, String symbolName)
    ...
    j = j + ",\"tierLabel\":\"" + JsonSafeString(tierLabel) + "\""
    j = j + ",\"symbol\":\"" + JsonSafeString(symbolName) + "\""
    j = j + ",\"flavor\":\"" + JsonSafeString(flavorText) + "\""
    j = j + ",\"boonText\":\"" + JsonSafeString(boonText) + "\""
    j = j + ",\"priceText\":\"" + JsonSafeString(priceText) + "\""
    j = j + "}}"
...
String Function GetDaedricMilestoneFlavor(String princeName, Int tierValue)
    ...
    elseIf (princeName == "Sheogorath" || princeName == "Sheo") && tierValue == TIER_CHAMPION
        return "Sheogorath names you the Madgod's own."
...
Function HandleBretonHiddenArtExposure(String reason)
    ...
    endIf
    ShowP2BookNotice(reason, "The Hidden Art", "Forbidden pages leave their mark on you.")
...
String Function GetRedguardSurveySectText()
    ...
    String standing = GetCurrentStandingLabel()
    if sectValue == REDGUARD_SECT_CROWN
...
String Function GetBretonSurveyText()
    ...
    text = text + " Fork: " + GetBretonDruidicForkLabel() + "."
