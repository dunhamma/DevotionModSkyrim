...
Function InitializePages()
    ...
    String[] configuredPages = new String[4]
...
Function DebugForceSelectedDaedricTier(Int tierValue)
    ...
    if ShowMessage("Force " + path.DeityName + " to " + tierLabel + " for controlled display proof?", True, "$Yes", "$No")
        ...
        ShowMessage(GetSelectedDaedricProofSummary(), False, "$OK", "")
        ForcePageReset()
        if PDV_Manager && tierValue > 0
            Int actualTier = path.GetStoredTier()
            if actualTier == tierValue
                TraceMcm(1, "Daedric force tier queued milestone replay for " + path.DeityName + " " + tierLabel)
                PDV_Manager.QueueDaedricMilestonePresentation(path, 0, actualTier, "mcm_force_" + tierLabel)
            else
                TraceMcm(1, "Daedric force tier milestone replay skipped for " + path.DeityName + ": requested " + tierValue + ", actual " + actualTier)
            endIf
        endIf
    endIf
EndFunction
