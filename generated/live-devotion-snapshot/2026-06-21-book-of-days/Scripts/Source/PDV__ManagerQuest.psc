...
Function SurfaceTransition(String eventClass, String surfaceKey, String direction, Int deityIndex = -1, String toneOverride = "", Bool repeatable = false, Bool headline = false)
    ...
    if line != ""
        ...
        AppendBookOfDaysEntry(line, Utility.GetCurrentGameTime() as Int, TransitionToneKey(eventClass, direction), ResolveTransitionJournalSymbol(eventClass, deityIndex), pinned)
    endIf
EndFunction
    ...
    if newTier != oldTier
        ...
        if deity == _activeDeity
            ...
            if surfaceTierUp && newTier > oldTier && NotifyTierUp(deity, newTier)
                ...
                ; Patron tier reaches journal too; the band in surfaceKey scopes the guard so
                ; each tier (Seeker/Devoted/Champion) journals once, Champion pinned.
...
; One neutral summary line for a day that saw devotion, lightly flavored per flagship race.
String Function BuildBookOfDaysDigestLine()
    Int originRace = GetPlayerOriginRaceIndex()
    if originRace == ORIGIN_NORD
        return "The day's devotions were kept, and settle with the dawn."
    elseIf originRace == ORIGIN_DUNMER
...
Function AppendBookOfDaysEntry(String line, Int gameDay, String tone, String symbol, Bool headlinePinned)
    if line == ""
        return
    endIf
    if tone == ""
...
; Remove one entry across all 5 parallel Book of Days lists at the same index.
