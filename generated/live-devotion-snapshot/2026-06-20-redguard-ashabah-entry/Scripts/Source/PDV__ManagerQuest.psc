...
Function HandleRedguardAshAbahMajorBurden(Form victimForm, Int eventType)
    ...
    RecordRedguardSectSignal(REDGUARD_SECT_ASHABAH, multiplier, "redguard_deathduty_major")
    ApplyRedguardAshAbahDutyRewards("redguard_deathduty_major")
    Trace(2, "Redguard Ash'abah major burden fired: unique undead defeat marks sect entry (eventType=" + eventType + ").")
EndFunction
...
Function HandleHoonDingBreakthroughKill(Form victimForm, Int eventType)
    ...
    if _activeDeity != PDV_HoonDing
        return
    endIf
    if eventType != 302 ; EVT_KILL_DRAGON
        return
    endIf

    Float multiplier = ConsumeDailyRepeatMultiplier("PDV.Signal.HoonDingDragon")
    if multiplier <= 0.0
        Trace(2, "HoonDing make-way (dragon) decayed out for today; no award.")
        return
    endIf

    AwardCuratedSignalScaled(PDV_HoonDing, PDV_HoonDing.SIGNAL_MAKE_WAY, victimForm, multiplier)
    Trace(2, "HoonDing make-way fired: breakthrough dragon kill multiplier=" + multiplier)
EndFunction
