...
Function TryNamiraFeedHeal()
    ...
    Trace(2, "Namira feed-heal fired tier=" + namiraTier + " mult=" + feedMultiplier + " restore=" + feedHeal)
EndFunction
...
; (user ruling 2026-06-20). Routine undead/burial duty is Noted (no stigma); only a
; MARKED burden (major tomb / necromancer operation / named-undead defeat / costly
; impurity choice -- the same reasons IsRedguardAshAbahBurden gates the sect on)
; raises the stigma and fires a marked-moment notice, paired with the Tu'whacca
; death-rite heal so the burden reads as earned. No service penalty, no voiced lines.
Function MarkRedguardAshAbahStigma(String reason)
    if !IsRedguardAshAbahBurden(reason)
        return
    endIf

    Int stigma = StorageUtil.AdjustIntValue(None, "PDV.Redguard.AshAbahStigma", 1)
    if stigma > 5
        stigma = 5
        StorageUtil.SetIntValue(None, "PDV.Redguard.AshAbahStigma", 5)
    endIf
    ...
    endIf
    Trace(2, "Redguard Ash'abah stigma marked reason=" + reason + " stigma=" + stigma)
EndFunction
