;/
    PDV_DaedricPath_Vaermina.psc
    PlayerDevotion - Vaermina Daedric path
    Generated from PDV_DaedricPrinceRecordContracts.json.
/;

Scriptname PDV_DaedricPath_Vaermina extends PDV_DaedricPathBase
...
Function DebugRunLiveSenderProof(String hookId)
    ...
    StorageUtil.SetStringValue(GetDeityForm(), "PDV.Daedric.Vaermina.LastLiveHook", hookId)
...
Function DebugRenouncePath()
    ...
    StorageUtil.SetIntValue(GetDeityForm(), "PDV.Daedric.Vaermina.Renounced", 1)
    if Msg_Exit
        Msg_Exit.Show()
    endIf
...
Function TraceControlled(Int level, String traceText)
    if GetDebugLevel() >= level
        Debug.Trace("[PDV] VaerminaPath: " + traceText)
    endIf
EndFunction
