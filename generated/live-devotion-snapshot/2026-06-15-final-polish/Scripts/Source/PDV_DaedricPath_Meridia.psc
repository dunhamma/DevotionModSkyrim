;/
    PDV_DaedricPath_Meridia.psc
    PlayerDevotion - Meridia Daedric path
    Generated from PDV_DaedricPrinceRecordContracts.json.
/;

Scriptname PDV_DaedricPath_Meridia extends PDV_DaedricPathBase
...
Function DebugRunLiveSenderProof(String hookId)
    ...
    StorageUtil.SetStringValue(GetDeityForm(), "PDV.Daedric.Meridia.LastLiveHook", hookId)
...
Function TraceControlled(Int level, String traceText)
    if GetDebugLevel() >= level
        Debug.Trace("[PDV] MeridiaPath: " + traceText)
    endIf
EndFunction
