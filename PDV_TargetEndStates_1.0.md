...
**Setup shape:** Explicit path choice at setup (Old Contract / Living Story / Exchange / Bandit Road). Each path is a meaningfully different experience. Path switching has real cost.

**Implementation state:** `PDV_State_BosmerPath` uses `OldContract = 0`, `LivingStory = 1`, `Exchange = 2`, `BanditRoad = 3`. First-run setup requires a choice; unset/corrupt fallback is `LivingStory`. `OldContract` path state is separate from `PactBound`, `GreenPactCompliance`, and `LapsedFromPact`; path orientation and active Y'ffre exclusivity are not the same variable. Non-Old-Contract incoherence drifts toward `LivingStory`, and path switching is explicit/destination-gated rather than automatic drift.
