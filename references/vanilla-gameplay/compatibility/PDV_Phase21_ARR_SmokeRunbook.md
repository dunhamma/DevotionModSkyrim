...
only — machine proof is green (`--matrix --check` PASS, 22 ARR cells + 6 core; all scripts compile 0/0;
core `--check` unchanged; 9 shrine-prayer Activators readback-verified).
...
5. **Shrine prayer (BOS + once/day +2)** — travel (load-door/fast-travel, NOT `coc`) to a man_ Daedric
   ...
   The 9 covered Princes: Azura, Vaermina, Molag Bal, Mephala, Mehrunes Dagon, Sheogorath, Namira,
   Sanguine, Hermaeus Mora. (Mehrunes Dagon rides the vanilla `ShrineMehrunes01` base — confirm only the
   intended shrine swapped, no unwanted bleed.)
...
## Authored reference
- **ARR matrix channel:** 22 cells / 20 keys / 19 quests across Vigilant, Glenmoril, Unslaad, Olenveld,
  ForgottenCity, SEC Saints&Seducers, and DAc0da — full list/valences in `PDV_QuestReactionMatrix_ARR.csv`
...
- **Shrine prayer:** 9 Activators + BOS swaps — `PDV_Phase21_ARR_ShrinePrayer.manifest.json`.
