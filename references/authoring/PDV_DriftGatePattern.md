...
| Generated Quest Reaction package | `PDV_QuestReactionCompatibility.manifest.json`, canonical CSVs, stage selectors, and locked adapter assets | Generated V2 catalogs, five-adapter FOMOD tree, XML, and exact receipts | The exact files in the verified archive | None; regenerate or rebuild | `stale-generated`; `pdv_quest_reaction_build --check` compares catalogs/tree/receipts and `--package --output` verifies normalized archive membership and bytes. |
