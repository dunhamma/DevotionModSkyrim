...
| Release-proof snapshot | Live `Devotion.esp` plus direct houseCARL readback | `PDV_HousecarlReleaseProof.json` | The live plugin and current load-order winner | None | Hash/readback comparisons are verification; `contestedRecordCount === 33` is `pin-only` until the set is re-derived. Backend absence fails a release build. |
