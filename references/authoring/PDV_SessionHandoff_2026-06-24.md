...
6. **Ancestral-spine parity build** (the big content workstream). **The Spine Stack Score is now BUILT** (`tools/pdv_spine_stack_score.mjs` + `PDV_SpineStackRegistry.csv` → `PDV_SpineStackScoreLedger.{md,csv}`; Argonian=100%, <70%=target; re-run to gate/measure the build). **Worst-first ranking (2026-06-24):** Nord 20% · Imperial 30% · Altmer 30% · Breton 36.7% · Orc 36.7% · Redguard 53.3% · Dunmer 63.3% (the 7 parity targets); Bosmer 73.3% / Khajiit 86.7% / Argonian 100% pass.
   ...
   - **6c — Specced-but-unemitted minuses (gap #4):** `tools/pdv_specced_minus_audit.mjs` (BUILT) machine-finds **18** curated-signal minuses defined+handled but **never emitted** (the manual audit caught only Redguard's) — Arkay×2, Boethiah, Hist×3 (`HIST_ABANDONMENT/CORRUPTION/VOID_OVERREACH`), Magnus×2, Malacath×3, Mephala, Stendarr×2, Trinimac, Tuwhacca (`DEATH_DUTY_ABANDONMENT`), Xarxes×2. Ledger: `PDV_SpeccedMinusLedger.md`. **Triage each:** wire an emit OR confirm the penalty is substrate-handled (then delete the dead deity-signal spec). NOTE: the "graduated minus stack" claim for Argonian/etc. is largely aspirational here, so the Score's `minus_stack` dim is over-credited in the registry — recalibrate after triage.
