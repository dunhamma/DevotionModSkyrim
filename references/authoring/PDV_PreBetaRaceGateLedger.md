...
Verdict: Conditional - prior route evidence remains useful, but the 2026-08-06 substrate, focus-emergence, observation, resonance, Portent, and Champion reward changes require the new in-game matrix before current runtime/manual passage
No-in-game status: Readback-Ready
...
Normal-session route: travel by foot -> complete an outdoor rest in an exterior bedroll or tent, or observe the moons -> trigger one Baan Dar, Rajhin, or Alkosh behavior-specific beat -> Survey/MCM readout.
Accepted hooks: completed non-interrupted sleep whose exterior status was captured at sleep start, moon observance route 10, Baan Dar route 90, Rajhin route 91, and Alkosh route 92. Historical route 33 activators remain QASmoke route proof, not the normal-play road-home rule.
Rejected hooks: required visual moon inspection, moon-sugar use, manual focus entitlement, fast travel loop, designated road-home anchors or circuits, generic inn/house sleep, interrupted sleep, sleep stop without captured start context, generic crime/theft, generic combat, generic dragon spam, and ordinary night stealth.
Anti-farm result: all approved lunar practices share one `+4` grant per 06:00 devotional day. The first completed outdoor rest that day receives one road-home toast and Book entry even if another practice already spent the grant; later outdoor rests that day add neither surface. Earlier same-anchor rejection, one-bed-camping rejection, same-day multiplier, and 1.00 -> 0.70 -> 0.49 evidence describe the superseded pre-2026-07-13 implementation. Current static gates cover the replacement rule; fresh runtime proof remains open.
...
Immersive hook result: QASmoke route proof exists; 2026-06-06 strict-manager checker passed for Khajiit lunar book route in Papyrus.0.log. Restarted packet passed the currently wired lunar book sources, wrong-origin rejection, generic-source silence, and reward-pending behavior. PDV_FLST_P2_KhajiitFocusedSources filled 2026-06-12 with MQ104 stage 160 (Alkosh route 92) and DA01 stage 100 (Azurah route); --check-source-fill PASS. Organic edge detection wired 2026-06-12 (record-wired, --check-khajiit-organic PASS): Baan Dar outnumbered-win/near-fatal-reversal combat sessions with adversity gate, Rajhin elegant-theft OnItemAdded detection (notable FLST + 200g floor, 7-day per-target cooldown), Alkosh named-dragon kill receiver (6 named bases, one-shot markers), generic-dragon weekly nudge, Paarthurnax chaos-aid negative, and Alkosh word-of-power dawn drip. See PDV_Phase20KhajiitImplementationCosting.manifest.json organicTriggerSurfaces. 2026-06-13 Baan Dar Champion survival capstone passed in game using the vanilla-shaped OnHit hidden effect at the 20% threshold and full-heal spell; all five focus gods now share the same dawn-owned Champion presentation. 2026-06-14 Rajhin elegant theft passed in the retained log at 01:16:16, including `RouteKhajiitRajhinElegantTheft complete`, signal 801, focus +25, ShadowDrift evidence, and per-target cooldown at 01:16:19. Alkosh word-of-power dawn drip passed at 02:14:25 -> 02:19:46 with event 343, scaled dawn piety, and `Khajiit Alkosh word-of-power drip awarded 2 of 2 new words`. Khajiit lunar books passed the focused P2 checker. The older `--race khajiit` QASmoke checker still fails on retained organic/book logs because it expects physical proof-activator marker strings; this is route-checker scope, not a beta-feel blocker for the packet.
Reward floor: exclusive Lunar Hardiness / Khajiit Lunar Road / Lunar Attunement packages at 1/25/75, plus broad road, sky, rest, and community practice without phase homework.
Reward ceiling: one exclusive lunar substrate tier plus one exclusive focused tier; matching the god in strength multiplies only the focused numeric package by 1.20. Portent and Baan Dar rescue are unamplified scripted Champion gifts.
Stack snapshot: 2026-06-14 pattern summaries captured KhajiitLunar metric/tier/phase/observance/road-home count, active focus, five weights, favor lane, lunar posture, and curse/Daedric state. Final retained snapshots include `focus=Rajhin; rj=100.00` at 01:53:03 and `focus=Alkosh; ak=75.00` at 02:19:46.
Runtime command: node .\tools\pdv_phase20_runtime_check.mjs --track p2-books --race khajiit
Next automatable action: complete direct readback and the Khajiit rebalance/static gate suite; record results in the 2026-08-06 runbook.
Deferred manual proof: execute `PDV_KhajiitLunarChampionRebalance_InGameRunbook_2026-08-06.md`, including outdoor sleep, boundaries, all fifteen packages, emergence/reorientation, eight god-strength slots, observation pools, Resonance, Portent classification, and stale Baan Dar rescue rejection.
Blocking follow-up: record the full current in-game matrix before restoring a runtime/manual Pass verdict. Final-world placement remains separate.
```
...
Race: Redguard
Lane type: P1 buildout packet
Verdict: Pass - manual/runtime packet 2026-06-19 (all 8 dimensions); final-world placement separate
Packet evidence (2026-06-19): origin 9, freshly-compiled build, all 8 dimensions. Expected build (ancestor-spine book 0001ACD1) PASS (top-left notice + Survey, no forced Prisma); sect no-flip PASS (one Crown signal did not flip default Forebear; 2-of-7-days + 3-day lock held); Far Shores token + daily cap PASS (Resist Magic +5% after a dawn pass; soft 0.7x daily credit decay); reward/stack snapshot PASS (focused Tu'whacca Champion T3 Resist Magic +20% + Fortify Health (Health Regen +25% leg INVALIDATED 2026-07-13 -> Fortify Health pool; re-prove Maximum Health rises) + Far Shores token + AncestorSpine-Seeker baseline riding with active patron; broad AncestorSpine T2 'Faithful' correctly absent under focus -> focused>broad, no overstack); vampire-cure re-entry PASS on substance (Papyrus log clean curse transitions vampire 0<->2 / werewolf 0<->1 via mcm_force; re-entry flags set; diegetic+Prisma+sound fired; per-race top-left notice subtle but handler ran, mirrors Nord's accepted 'tester missed the visual'); wrong-origin rejection PASS (origin 8 book read -> no Redguard movement); generic-source silence PASS for the Arkay sub-check (Arkay shrine ZERO Tu'whacca/PDV movement; cure-only substitution guard holds); Survey/status clarity PASS (sect/patron/tier/Far Shores/curse-cycle in Yokudan voice). Active patron=Tu'whacca confirmed. Non-blocking tracked follow-ups (NOT failures; consistent with the 7/10 commit-709eb78 stubbed-but-tracked precedent): (A) vampire EARN-HALT specced-not-built -- GetCurseGainMultiplier=1.0 for all Yokudan deities (gain-halt Imperial-only), piety accrued full-rate while vampire (log-proven; own-patron, no wrong-origin/generic scoring) -> content pass; (B) HoonDing/Leki generic-combat day-to-day leak (R7) -- generic kills (events 1/2) + discovery (event 345) feed HoonDing/Leki via the day-to-day likes/dislikes layer (hard-capped 3/day), contradicting their reject-generic-combat spec; corrective CSV edit APPLIED (3 kill rows removed; make-way/discipline likes kept for Champion-pace parity since curated make-way is weekly-capped) now FIXED 2026-06-19 (LoadRowsForDeity regenerated + LIKES_DISLIKES_VERSION 8->9 -- the version bump clears the superset incl. events 1/2 then reloads; compile 0/0, verifier FAIL=0; awaiting in-game reconfirm of zero HoonDing/Leki movement on generic kills); byte-identical kill rows ship live+unfixed on already-passed Orc (Malacath 1/2) and Nord (Talos/Tsun/Stuhn/Shor 2), so route/state-layer-clean by the operative precedent; (C) Arkay shrine-blessing neutralization residual -- still prints 'Blessing of Arkay added' + grants cure-only ability though stat boon suppressed (no piety leak); manifest runtime-proven -> cosmetic polish.
No-in-game status: Readback-Ready
...
Packet evidence (2026-06-19): Hidden Art book route PASS (RouteBretonHiddenArtExposure complete x3; notice fires on first read, missed among rapid identical reads -> per-book distinct notices added this session); WitchcraftExposure decay PASS (-1/dawn confirmed in log 24->23->22->21, no longer a one-way ratchet; bands notorious>=100/known>=50/suspected>=25/hidden<25; the 75-point three-book max is known/near-rupture, not notorious); wrong-origin rejection PASS; generic-source silence PASS (no movement from generic spellcasting/artifact/faction-join). 2026-06-28 post-fix Survey retest PASS: screenshot showed the three-book 75-exposure state as "known" and "close to rupture," not "notorious" or "full commitment"; Prisma panel summary/history should be judged against the same corrected >=100 notorious threshold on new panel opens. 2026-06-28 dawn-decay retest PASS: Papyrus.0.log captured `Breton WitchcraftExposure passive decay -> 74`, `-> 73`, and `-> 72`; Survey stepped down to the known/Vigilants line; Book of Days produced one dawn digest, not one passive-decay milestone per tick. Note: "Standing: distant" is the devotional piety tier (GetPublicTierBand), separate from the exposure axis. Optional creed-loss ExposureRupture spell needs exposure>=100 (unreachable via the 3 one-shot books, max 75) -> needs an MCM band setter, PENDING. Deferred-by-design (NOT failures, no V1 emitter): Knight's Road breach hooks (Thieves Guild/DB/innocent-kill), Green Way DruidicStanding degradation, tradition-differentiated vampire. Reward-magnitude rebalance shared with task #9.
...
Survey/status result: PASS for the wired Hidden Art lane. 2026-06-28 screenshot confirmed the corrected 75-exposure Survey wording after reload; MCM/panel summary surfaces are expected to use the same live state labels on new opens. Knight's Road and Green Way remain deferred-by-design, not failed.
Reward ceiling: one tradition spine plus one focused patron; other tracks modify, gate, rupture, or penalize.
Stack snapshot: tradition, WitchcraftExposure, KnightlyVowIntegrity, DruidicStanding, patron focus, active favor, curse fork, Daedric modifiers. 2026-06-28 snapshot showed expected Breton inherited ward/resist-magic and no ExposureRupture below 100 exposure, but also exposed the broad diegetic helper `Devotion Revelation Aura`; keep stack snapshot open until the helper-aura HideInUI ESP fix is applied and retested.
...
Deferred manual proof: expected/edge stack audit, manual feel note across the broader tradition design, and optional ExposureRupture proof once an MCM band setter exists.
Blocking follow-up: no Breton reward expansion until the expected/edge stack audit and ExposureRupture edge are proven; Hidden Art route, Survey, rejection, and asset-status proof are recorded for the current packet.
```
