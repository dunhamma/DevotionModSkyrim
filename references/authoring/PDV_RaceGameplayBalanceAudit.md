...
|---|---|---|---|---|
| Altmer | First focused pass complete; implementation-spec closeout landed. | First focused pass complete; implementation-spec closeout landed. | `Watch`; former P0 gap is closed, the first source scaffold compiles, `PDV_State_AltmerCrisis` is record-wired, the first two favor spell records are wired, and four trigger proof ACTI base records are wired, but Lorkhan pressure and crisis pacing must be playtested carefully. | Place/attach the proof triggers, then promote Lorkhan tags, Exiled vampire micro-path, rejected-surface assertions, and first crisis/favor playfeel from source/record scaffold into runtime proof. |
| Argonian | First focused pass complete. | First focused pass complete. | `Thin`; architecture locked, but Hist/People reward wealth needs support before Void expansion. | Build Hist/People backlog: Hist sap, water/rest maintenance, bed-of-choice proof, community recognition, Arkay death-rite reactions, and curse posture readback. |
| Orc | First focused pass complete. | First focused pass complete. | `Watch`; Stronghold/crafting are rich, City and Legion/Exile need dynamic situational parity. | Cost life-mode state, mode gates, quality forge filters, self-made community, service milestones, dignity moments, and rejected-hook tests. |
| Redguard | First focused pass complete. | First focused pass complete. | `Watch`; content is rich, but martial/death-duty hooks can flatten sect distinction. | Cost sect state/readback, Far Shores token, Crown/Forebear/Ash'abah favor hooks, HoonDing cap, Ash'abah stigma surfacing, and rejected-hook tests. |
| Bosmer | First focused pass complete; non-hunter parity proof packet landed. | First focused pass complete; non-hunter parity proof packet landed. | `Watch`; path system is runtime-proven and the non-hunter favor table is now source/record-wired, but Old Contract/hunter proof density still needs playfeel comparison once placed. | Place/prove the eight Living Story, Exchange, Bandit Road, and Old Contract favor proof ACTIs, then run runtime playfeel against the Phase 9 hunter baseline and the Green Pact tag-layer gate. |
| Khajiit | First focused pass complete; moon/road/focus proof packet landed. | First focused pass complete; moon/road/focus proof packet landed. | `Watch`; lunar/road system is distinctive and now source/record-wired, but phase timing can become chores and Khenarthi/Azurah are easiest. | Place/prove the six moon, road-home, Baan Dar, Rajhin, and Alkosh proof ACTIs, then run cadence/playfeel checks for lunar chore pressure and focus split. |
...
| Priority | Work | Why |
|---|---|---|
| P0 | Altmer proof-placement to runtime proof | Implementation-spec is closed; `PDV_Phase20AltmerImplementationCosting.manifest.json` defines the buildable state/content/rejected-surface tests, manager/EventBus/EventTypes/receiver routes now compile, `PDV_State_AltmerCrisis` is record-wired, the first two favor spell records are wired, four trigger proof ACTI base records are wired, three curse/exile message records are wired, and the four QASmoke proof references pass readback. The next gate is runtime proof and playfeel tuning. |
| P1 | Argonian, Orc, Redguard, Bosmer, Khajiit runtime proof | These are the main parity risks: thin non-Sithis support, City/Legion parity, sect distinction, non-hunter payoff, and lunar chore pressure. All five child packets are now source/record/proof-placement wired under `--strict-phase20-race-costing`; the remaining gate is runtime proof, negative-hook proof, and playfeel tuning. |
...
| Altmer | Coherence under Lorkhan pressure; Auri-El foundation plus secondary focus. | `ThalmorAlignment`, Lorkhan pressure, crisis-of-faith, Exiled vampire micro-path. | Excellent conceptual friction; implementation-spec is closed, source compiles, crisis state is record-wired, first two favor spell records are wired, and four trigger proof ACTI base records are wired. | Can still feel punitive if crisis pacing fires too often or if normal Skyrim play is over-taxed. | Place/attach triggers, then prove rejected-surface runtime tests, one-active-favor gates, Exiled vampire cap, and main-quest + daily-life pressure playfeel. |
...
1. **Build from the implementation-costing backlog.** Continue by placing the wired Altmer, Argonian, Orc, Redguard, Bosmer, and Khajiit proof triggers, then run their runtime proof, rejected-surface tests, and first playfeel proof before adding more broad race content.
