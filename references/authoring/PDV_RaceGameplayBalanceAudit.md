...
- `race-sheets/PDV_DaedricContent_Manifest.md` now has draft rows for Boethiah plus the remaining fifteen Skyrim-present Princes. Those rows are content-lock inputs, not promotion approval: D-15..D-18 lock stigma handling, Hircine/Molag Bal curse-access variation, Prince authoring order, and per-Prince content-ready criteria; promotion still needs per-row CAT-6 proof, record readback, runtime or display proof, and stack/Survey legibility.
...
4. **Close Daedric proof-path blockers before broad Prince cloning.** Section 11.6 D-12..D-18 now locks roster shape, mixed recovery, reduced cross-Prince hostility, stigma handling, Hircine/Molag Bal curse-access templates, authoring order, and per-Prince content-ready criteria. The remaining blocker is proof: per-row CAT-6 promotion, record readback, runtime or display proof, and stack/Survey legibility. Prioritize Princes with high race-crossing risk: Hircine, Molag Bal, Nocturnal, Hermaeus Mora, Malacath, Azura, and Mephala.
