...
| **The Code Holds** (survival beat) | `PDV_SPEL_OrcCodeHolds` + `PDV_SPEL_OrcCodeHolds_Devoted` (gating flags; not cast) | **LIVE/readback-clean 2026-06-14; flat-restore conversion live + spec-synced 2026-06-23.** On surviving combat after dropping below 20% health: a flat scripted `RestoreActorValue` in `TryOrcCodeHolds`, once per combat. Seeker +40 Health; Devoted/Champion +60 Health + 30 Stamina. +0.5 Malacath piety/combat. The old HealRate regen MGEF is no longer cast (Requiem-swallowed). Quiet (effect only). |
