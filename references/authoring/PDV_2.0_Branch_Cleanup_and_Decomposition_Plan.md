...
>
> **Implementation checkpoint (2026-08-20):** ORIGIN, RECOGNITION, PRISMA, and DEBUG are extracted on `feature/v3-big-update`. PRISMA resides in `PDV_PrismaPresenter` (114 functions; 48 public / 66 private), DEBUG in `PDV_DebugRuntime` (136 functions; 111 public / 25 module-internal), and per-race presentation is supplied by the accepted ORIGIN hook. Manager VMAD cleanup is closed at 455 unique fills, the generated module-contract authority is current, and the trigger-first static optimization pass is implemented. Clean 119-script compile, extraction-specific gates, direct houseCARL VMAD/readback, SEQ, and the full verifier are green. Fresh-game/player-surface proof, deterministic profiler comparison, owner copy, embedded-Prisma font-payload proof, and package acceptance remain independent later buckets.
>
