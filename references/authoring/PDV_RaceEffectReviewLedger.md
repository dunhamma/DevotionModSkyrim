...
|------|--------|--------------------|--------------------------|
| Altmer | Pending (rite L5 ratified) | Auri-El/Magnus scholar floor, crisis/recovery ceiling | What is the quiet positive floor before crisis pressure? Which favors are one-active only? How do Exiled vampire and werewolf halt suppress rewards? **RITE (L5 Disciplines of Return) RATIFIED 2026-06-25** -- 4 schools at -5% cost, Papyrus built (commit 2c3e26e, inert until records bind), records turnkey in PDV_6f_RiteBuild_GateSpec_2026-06-25.md. Tranche signatures/pilgrimage/contemplations still pending. |
...
| Orc | Pending (rite L5 ratified) | Stronghold/city/Legion/exile labor dignity | Which effects reward quality forge/service without raw crafting or combat loops? How does exile remain playable without free Malacath stacking? **RITE (L5 Trial of Iron) RATIFIED 2026-06-25** -- shipped magnitudes accepted (Tusk/Shield/Hammer +5, Yoke +15; SPEL+MGEF already exist), Papyrus built (commit 14d9ac3, inert), only PDV_MESG_Orc_TrialOfIron to author -- see PDV_6f_RiteBuild_GateSpec_2026-06-25.md. Tranche Watchers/Hearth-Held/Code-Holds/Four-Holds still pending. |
| Redguard | Pending | Crown/Forebear/Ash'abah/Far Shores posture | 2026-06-19 packet PASS informed this: HoonDing cap = weekly (1/7d, route handler wired); generic-undead/combat farming surfaced as the HoonDing/Leki day-to-day leak (Finding B) -> fix APPLIED 2026-06-19 (kill rows removed + LoadRowsForDeity regen + VERSION 8->9 + compile/verify clean; awaiting in-game reconfirm); route/state layer is clean. Open: where does HoonDing cap escalation stop? **RITE (L5 Remembering of Names) RATIFIED 2026-06-25** -- Blade OneHanded+5 / Road StaminaRateMult+8% / Rest HealRate+5% (both PeakValueModifier) / Harvest Speech+5% (barter->Speech reconciliation, Bosmer Keeper precedent); Papyrus built (commit 9b73bf2, inert), 4 SPEL+MGEF + PDV_MSG_RedguardRemembering to author -- see PDV_6f_RiteBuild_GateSpec_2026-06-25.md. Tranche dreams/sword-tending/signatures/Halls still pending. |
...
`DAEDRIC_PRINCE_EFFECT_MODEL[stem].prices` in
`tools/lib/pdv_daedric_effect_model.mjs`. Invariants now enforced:
...
| Prince | old price | current price | reason |
|---|---|---|---|
| Azura | MagickaRateMult | Stamina | same-axis fix; vigil of foresight wearies |
| Sheogorath | MagickaRateMult | Restoration | same-axis fix; madness erodes self-restoration |
| Sanguine | StaminaRateMult | Magicka | same-axis fix; dissipation dulls the mind |
| Clavicus Vile | Speechcraft | Magicka | same-axis fix; the bargain drains vital spark |
| Vaermina | StaminaRateMult | Health | corrupted sleep → poor recovery |
| Nocturnal | Speechcraft | Restoration | the Empty Night claims vitality |
| Hircine | Speechcraft | Health | the hunt's toll; bites the melee build |

Kept (already distinct + real cost): Boethiah/Mephala/Namira = Speechcraft (social
stigma), Malacath = SpeedMult, Dagon = DamageResist, Molag Bal = Health,
Meridia = Illusion, Hermaeus Mora/Peryite = Stamina. Price flavor text is
