...
- **HONEST CORRECTION — the input is thinner than the 2026-06-09 draft claimed.** That draft asserted "the per-deity disposition net scalar is already baked in." Per `PDV_DeityLikesDislikesMatrix.md §2/§10`, the day-to-day faucet that feeds `clampedToday` is **proven for Kyne combat events only** (the data-driven `ScoreFromTable` path, events 1–4). The non-combat 300+ vocabulary (rest-under-open-sky, learn-word-of-power, craft, read…) has **no Story Manager receivers and no router classifier yet** — both TODO. So today, Kyne's mood moves mostly from *kills*, and the "honor the wild" feel that should drive it is not yet wired. This does **not** block LD-P1 (mood still works off the live signals), but it **bounds liveliness** and makes the faucet-breadth work (matrix §10 steps 3–4) a **soft dependency** for LD-P1 to feel like the north star.
