;/
    PDV_DaedricPath_Malacath.psc
    PlayerDevotion - Malacath Daedric path
    Generated from PDV_DaedricPrinceRecordContracts.json.
/;

Scriptname PDV_DaedricPath_Malacath extends PDV_DaedricPathBase
...
Function DebugRunLiveSenderProof(String hookId)
    ...
    StorageUtil.SetStringValue(GetDeityForm(), "PDV.Daedric.Malacath.LastLiveHook", hookId)
...
Function DebugRenouncePath()
    ...
    StorageUtil.SetIntValue(GetDeityForm(), "PDV.Daedric.Malacath.Renounced", 1)
    if Msg_Exit
        Msg_Exit.Show()
    endIf
...
Function TraceControlled(Int level, String traceText)
    if GetDebugLevel() >= level
        Debug.Trace("[PDV] MalacathPath: " + traceText)
    endIf
EndFunction
