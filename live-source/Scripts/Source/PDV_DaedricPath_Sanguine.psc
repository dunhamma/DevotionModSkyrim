;/
    PDV_DaedricPath_Sanguine.psc
    PlayerDevotion - Sanguine Daedric path
    Generated from PDV_DaedricPrinceRecordContracts.json.
/;

Scriptname PDV_DaedricPath_Sanguine extends PDV_DaedricPathBase
...
Function DebugRunLiveSenderProof(String hookId)
    ...
    StorageUtil.SetStringValue(GetDeityForm(), "PDV.Daedric.Sanguine.LastLiveHook", hookId)
...
Function DebugRenouncePath()
    ...
    StorageUtil.SetIntValue(GetDeityForm(), "PDV.Daedric.Sanguine.Renounced", 1)
    if Msg_Exit
        Msg_Exit.Show()
    endIf
...
Function TraceControlled(Int level, String traceText)
    if GetDebugLevel() >= level
        Debug.Trace("[PDV] SanguinePath: " + traceText)
    endIf
EndFunction
