;/
    PDV_DaedricPath_Mora.psc
    PlayerDevotion - Hermaeus Mora Daedric path
    Generated from PDV_DaedricPrinceRecordContracts.json.
/;

Scriptname PDV_DaedricPath_Mora extends PDV_DaedricPathBase
...
Function DebugRunLiveSenderProof(String hookId)
    ...
    StorageUtil.SetStringValue(GetDeityForm(), "PDV.Daedric.Mora.LastLiveHook", hookId)
...
Function DebugRenouncePath()
    ...
    StorageUtil.SetIntValue(GetDeityForm(), "PDV.Daedric.Mora.Renounced", 1)
    if Msg_Exit
        Msg_Exit.Show()
    endIf
...
Function TraceControlled(Int level, String traceText)
    if GetDebugLevel() >= level
        Debug.Trace("[PDV] MoraPath: " + traceText)
    endIf
EndFunction
