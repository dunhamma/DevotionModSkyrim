;/
    ...
    PlayerDevotion - Altmer ancestral-spine substrate
    -----------------------------------------------------------------------
    Always-on Altmer identity layer. Accepted dawn, orthodoxy, study, and
    dream routes deepen Aldmeri heritage standing and band into a highest-slot
    boon without requiring an active patron.
    -----------------------------------------------------------------------
/;

Scriptname PDV_Substrate_AltmerAncestor extends PDV_SubstrateBase

Float Property HeritageStandingDelta = 5.0 Auto
...
Function RecordHeritageStandingScaled(Float multiplier, String reason)
    Float delta = HeritageStandingDelta * ClampSignalMultiplier(multiplier)
    ...
    StorageUtil.SetIntValue(GetSubstrateForm(), "PDV.Substrate.AltmerAncestor.LastMaintenanceDay", Utility.GetCurrentGameTime() as Int)
    AdjustMetric(delta, "altmer_heritage_" + reason)
    Trace(2, "Heritage standing recorded with delta " + delta)
EndFunction
...
Function ProcessHeritageDawn(Bool curseActive, String reason)
    ...
    Int lastMaintenanceDay = StorageUtil.GetIntValue(GetSubstrateForm(), "PDV.Substrate.AltmerAncestor.LastMaintenanceDay")
