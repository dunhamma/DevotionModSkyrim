...
**Status:** locked 2026-04-30 by Aaron.
**Scope:** Binding standard for every Claude Code skill shipped under the Housecarl project (`.claude/skills/<name>/SKILL.md` inside the Housecarl repo, plus any skills bundled into the MCP plugin distribution).
**Phase 2 use:** Bucket old `<old-repo>/.claude/skills/*` against this standard. Drop or REBUILD any skill whose description doesn't conform; do not PORT skills wholesale.
...
**Two-surface scope (post-Q5.5).** This standard governs both (a) Housecarl team authoring of shipped skills and (b) user-side skill authoring mediated by the AD-4 `skill-authoring` meta-skill specialist. Per Q5.5, the v1.0 trust stack relies on (1) bundled standards in the specialist's `references/` (this standard is one of them), (2) the specialist invoking Anthropic's `skill-creator` skill with Housecarl standards as project-specific context, and (3) eval-set discipline per §6.3. There is no runtime validator. Both surfaces apply this standard's rules; both run skill-creator's description-optimization loop where environmentally feasible (§6.4).
...
- **Folder name == `name:` frontmatter value.** If the frontmatter omits `name`, the folder name becomes the slash-command per Claude Code's loader. Always omit `name:` and let the folder drive it — there is no second source of truth to drift.
...
- **Prefer:** modlist names (Authoria, Lorerim, Wildlander, Septim's Skyrim), plugin conflicts, ESP merges, BSA extracts, NPC outfit conflicts, leveled-list patches, override resolutions, crash logs, Papyrus log lines, MO2 instance names, USVFS overlays, load order positions
...
- **Per-MO2-instance install paths** (post-Q5/Q6 install-bundle delivery): user-side AD-4-authored skills (per §1 dual-surface scope) reference paths inside the housecarl install at `<housecarl-install>/...` (e.g., `<housecarl-install>/CLAUDE_user.md`, `<housecarl-install>/.claude/skills/<user-slug>/`) or the broader MO2 instance at `<MO2-instance>/...` when relevant. Do NOT use absolute paths — the install location varies per user. Dev-team-side (in-repo) skills continue to use repo-relative paths per the bullets above.
...
**v1.0 worked example: AD-4 meta-skill cluster (Q5 lock 2026-05-01).** The router + 2-procedural-specialists pattern is the canonical v1.0 implementation: `modlist-authoring` (router — decides "skill or knowledge file?" based on user intent) + `skill-authoring` (procedural specialist; output = SKILL.md authored against this standard's §3–§5 rules; bundles HOUSECARL_SKILL_AUTHORING.md + HOUSECARL_NAMING.md verbatim in `references/`) + `knowledge-file-authoring` (procedural specialist; output = `CLAUDE_user.md` authored against HOUSECARL_DOC_HYGIENE.md; bundles HOUSECARL_DOC_HYGIENE.md + HOUSECARL_NAMING.md verbatim in `references/`). Names are purpose-based per HOUSECARL_NAMING.md §9.2 — context provided by parent housecarl folder, no `housecarl-` prefix. Phase 3.6 authors these three artifacts; their bodies and eval sets (per §6.7 author-output methodology) are the reference-shape for v1.x router-plus-specialist clusters.
...
| Skill encoding a hardcoded modlist's facts ("in Authoria, the Lydia override is plugin X") | Either an addon-style data file under `<modlist>/.claude/` (per the per-modlist convention from Claude_MO2; codified in `HOUSECARL_REPO_LAYOUT.md`, P1.7) or a project memory entry. Not a shipped skill. |
...
**User-side AD-4 eval workflow note (post-Q5/Q6 install-bundle delivery; Q5.6 promotion):** When the AD-4 `skill-authoring` specialist authors a user-side skill (at `<housecarl-install>/.claude/skills/<user-slug>/`), the §6.3 trigger eval + §6.6 outcome eval (if Q3-scope) + §6.7 author-output eval all apply — but the user is on Windows, where `run_loop.py` is broken per the caveat above. Until a Windows-compatible eval runner ships, user-side AD-4 eval workflows fall back to §6.5 manual validation. This is a v1.0 trust-stack gap (Q5.5 substituted eval discipline for the runtime validator; user-side eval discipline is environmentally constrained on Windows). Phase 3.6 dev/research session may surface a Windows-compatible eval runner as a v1.0 ship dependency or accept the §6.5 fallback for v1.0 with a v1.x follow-up. Decision is Aaron's at Phase 3.6 lock.
...
**Scope:** Skills subject to Q3 conformance scoring include investigation-flow skills (`crash-diagnostics`, `mod-dissection` per Q11 lock), procedural-write skills (any skill that prescribes a write operation that produces a user-visible artifact), and the AD-4 meta-skill cluster (`modlist-authoring` router + `skill-authoring` + `knowledge-file-authoring` specialists per Q5 lock — though those have their own author-output eval methodology in §6.7).
...
### 6.7 — Author-output eval methodology (AD-4 meta-skills only)

Per Q5.6 promotion (vision-alignment 2026-05-01), skills that author other skills or knowledge files — the AD-4 meta-skill cluster (`modlist-authoring` router + `skill-authoring` + `knowledge-file-authoring` specialists per Q5 lock) — require a **second-axis eval set beyond §6.2's trigger eval and §6.6's outcome correctness eval**. The author-output eval scores the *artifact the skill produces* against this standard's §3–§5 rules + HOUSECARL_NAMING.md + HOUSECARL_DOC_HYGIENE.md as applicable.

**Scope:** Only the 3 AD-4 meta-skills require this eval methodology. Operational-archetype skills (per §5.2) and investigation-flow skills (per §5.2 fourth archetype) do not — their outputs are session reasoning, not authored artifacts.

**Required composition:**
- ≥ 5 author-input cases ("user wants to author a skill that does X for modlist Y" or "user wants a knowledge file capturing Z facts about their Authoria modlist") with reference outputs reviewers consider standards-conforming
- Reference outputs hand-authored at Phase 3.6 dev/research session by Aaron + conductor pair
- Scoring rubric: per-rule pass/fail walking the §8 reviewer checklist applied to the *authored output*, not the meta-skill itself
- Eval set archived at `<meta-skill>/evals/author_output_eval.json` per the §6.3 checkpoint 18 pattern (separate from trigger eval and Q3 outcome eval)

**Pass threshold:** [Aaron-decision pending — default candidate: ≥ 80% per-rule pass rate across the 5 cases (analogous to §6.3's 80% recall threshold), with hard-fail on certain rule classes (e.g., the §3.4 forbidden-content list — any forbidden content in authored output is ship-blocker). Aaron may relax or tighten per Phase 3.6 dev/research outcome.]

**Why this is v1.0, not v1.x:** Per Q5.5, the v1.0 trust stack drops the runtime validator. Eval-set discipline at the meta-skill level is the v1.0 trust mechanism for AD-4 author-output correctness. Promoting author-output eval methodology from v1.x to v1.0 is the trust substitution; without this gate, Aaron has no signal that the AD-4 cluster authors standards-conforming outputs in the field.

**Cross-references:** §1 dual-surface scope paragraph (Q5.5 trust stack), §6.3 (trigger eval — distinct concern), §6.6 (Q3 outcome eval — distinct concern), VISION_ALIGNMENT_PROGRESS.md §Q5 + §Q5.5 + §Q5.6.
...
2. ☐ Frontmatter omits `name:`; folder name is the source of truth.
...
A skill failing any of items 1-15 fails review on the spot. Items 16-18 may be deferred only with explicit Aaron sign-off and a tracked follow-up.
