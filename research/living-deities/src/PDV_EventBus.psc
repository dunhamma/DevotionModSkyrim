...
; LD-P1 Living Deities: dream omens ride OnSleepStart (PDV_PlayerEvents).
Function RouteSleepDreamOmen(Actor playerRef)
    if !PDV_Manager
        Trace(1, "RouteSleepDreamOmen skipped: PDV_Manager not assigned.")
        return
    endIf

    PDV_Manager.HandleSleepDreamOmen()
    Trace(2, "RouteSleepDreamOmen complete.")
EndFunction
...
Function RouteKhajiitBaanDarRoadTrick()
    ...
    Trace(2, "RouteKhajiitBaanDarRoadTrick complete: " + eventType)
EndFunction
...
Function RouteKhajiitRajhinElegantTheft()
    ...
    PDV_Manager.HandleKhajiitRajhinElegantTheft("eventbus_" + eventType)
    Trace(2, "RouteKhajiitRajhinElegantTheft complete: " + eventType)
EndFunction
...
Function RouteKhajiitAlkoshDragonOrder()
    ...
    Trace(2, "RouteKhajiitAlkoshDragonOrder complete: " + eventType)
EndFunction
    ...
    while i < count
        ...
        if deity
            ...
            if delta != 0.0
                ...
                ; LD-P1: faucet-binding demand fulfillment rides this existing
                ; routing point - the only seam that knows the Int eventType
                ; (M4 3.5). Positive scores only.
                if delta > 0.0
                    PDV_Manager.NotifyDemandFaucetSignal(deity, eventType, targetRef)
                endIf
