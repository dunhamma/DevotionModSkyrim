...
; The curse-gated Hircine deity face (second face; the path actor above is the
; onboarding/stigma/cure layer and stays unchanged). CK-wired in Block C.
PDV_Deity_Hircine Property PDV_HircineDeity Auto
; Active patron's mood band mirror for CK Condition / MGEF reads (script-poll
; reads only; refreshed at dawn and on band-cross). 0=Wroth 1=Cool 2=Pleased 3=Exalted.
GlobalVariable Property PDV_GLO_PatronMoodBand Auto
; Victim races that satisfy the "great_beast" demand event filter (Hircine's
; Hunt). Unwired (None/empty) means the filter never passes - honest no-op.
FormList Property PDV_FLST_DemandGreatBeasts Auto
...
Int Property LIKES_DISLIKES_VERSION = 5 AutoReadOnly
Int Property PRINCE_LD_VERSION = 2 AutoReadOnly
...
; --- LD-P1 Living Deities (research slice) ---
; Runtime tunables JSON compiled from references/authoring/PDV_DeityMood.csv /
; PDV_DemandTable.csv / PDV_OmenProfile.csv by tools/pdv_living_deities_compile.mjs.
String Property LIVING_DEITIES_FILE = "PlayerDevotion/PDV_LivingDeities" AutoReadOnly
Int Property MOOD_BAND_WROTH = 0 AutoReadOnly
Int Property MOOD_BAND_COOL = 1 AutoReadOnly
Int Property MOOD_BAND_PLEASED = 2 AutoReadOnly
Int Property MOOD_BAND_EXALTED = 3 AutoReadOnly
; Default band thresholds (asymmetric, M2 locked): Wroth [-100,-40) Cool [-40,10)
; Pleased [10,55) Exalted [55,100]. Per-deity JSON values win when present.
...
Function ApplyDeityReaction(String deityName, String valence, String intensity, String magnitude, String sourceTag, Bool isFaucet, Form sourceForm)
    ...
    ; LD-P1: quest-matrix demand fulfillment rides this existing routing point
    ; (M4 section 3.5). Keyed by deity NAME so a path-routed reaction (e.g.
    ; Hircine's DA05 rows) can still fulfill the deity face's pending demand.
    if amount > 0.0
        NotifyDemandQuestTag(deityName, sourceTag)
    endIf
EndFunction

PDV_DeityBase Function GetQuestReactionDeity(String deityName)
    ; LD-P1 routing guard: once the curse-gated Hircine deity face joins
    ; PDV_FLST_AllDeities it shares the name "Hircine" with the path actor.
    ; Quest-matrix rows must KEEP routing to the path (stigma/onboarding layer)
    ; exactly as shipped; the deity face hears about matrix tags only through
    ; NotifyDemandQuestTag. Do not let the FLST name scan steal this route.
    if deityName == "Hircine" && PDV_HircinePath
        return PDV_HircinePath
    endIf
...
Bool Function PushDevotionPanel()
    ...
    if _activeDeity
        ...
        ; Piety stays 0 for substrate races ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â there is no single scoring float.
...
String Function GetPanelPatronNote()
    ...
    endIf
    ; GetPlayerMcmModeLine handles all races ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â active patron, substrate, and
    ; state-track modes ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â so it works for both deity and quasi-patron cases.
...
String Function GetPanelActsJson()
    ...
    ; when there is no scoring patron ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â gives the player their mode at a glance.
    if !_activeDeity
        Int originRace = GetPlayerOriginRaceIndex()
...
Function RunDawnConsolidateScratch()
    ...
    while i < count
        ...
        if deity
            ...
            ; LD-P1: mood EWMA folded into this loop so it reads clampedToday
            ; BEFORE PietyToday is zeroed above (M4 section 3.2).
            RunDawnUpdateMoodForDeity(deity, clampedToday)
...
Function RunDawnProcessDemandsNoop()
...
Function HandleCurseStateTransition(Int oldState, Int newState, String reason)
    ...
    ; LD-P1: the Hircine deity face zeros its own mood/demand state on cure.
    ; The path actor's transitions (stigma/residue) are untouched - no double-fire.
    if PDV_HircineDeity
        PDV_HircineDeity.HandleCurseTransition(oldState, newState, reason)
    endIf
...
; Derive a typed "curse" Prisma event from an oldÃƒÂ¢Ã¢â‚¬Â Ã¢â‚¬â„¢new curse-state transition.
; Symbol names (curse-vampire, curse-werewolf) fall back to "journal" until
; the glyph design pass lands ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â no rendering breakage in the meantime.
...
; Short race-specific context phrase ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â feeds the UI's listText fallback and any
...
;/ =====================================================================
    LD-P1 Living Deities engine (research slice)
    ---------------------------------------------------------------------
    Mood EWMA + bands, active patron pool filter, band-cross omen dispatch,
    demand offer/fulfill/expire, dream omens, and the patron mood-band
    mirror. Spec: references/research/living-deities/04_living_deities_
    architecture.md (revised 2026-06-10). Tunables ride the compiled JSON
    (LIVING_DEITIES_FILE); deities absent from that JSON still get a mood
    (default alpha/thresholds) but are never demand/omen-eligible.
   ===================================================================== /;

Float Function GetDeityMood(PDV_DeityBase deity)
    if !deity
        return 0.0
    endIf
    return StorageUtil.GetFloatValue(deity as Form, "PDV.Mood")
EndFunction

Int Function GetDeityMoodBand(PDV_DeityBase deity)
    if !deity
        return MOOD_BAND_COOL
    endIf
    EnsureMoodState(deity)
    return StorageUtil.GetIntValue(deity as Form, "PDV.Mood.Band")
EndFunction

String Function GetMoodBandLabel(Int bandValue)
    if bandValue == MOOD_BAND_WROTH
        return "Wroth"
    elseIf bandValue == MOOD_BAND_PLEASED
        return "Pleased"
    elseIf bandValue == MOOD_BAND_EXALTED
        return "Exalted"
    endIf
    return "Cool"
EndFunction

; Default int for an uninitialized StorageUtil key is 0 (= Wroth), so the band
; key must be seeded to Cool before any read or the first dawn would register
; a spurious Wroth->Cool cross.
Function EnsureMoodState(PDV_DeityBase deity)
    Form deityForm = deity as Form
    if StorageUtil.GetIntValue(deityForm, "PDV.Mood.Init") != 1
        StorageUtil.SetIntValue(deityForm, "PDV.Mood.Init", 1)
        StorageUtil.SetFloatValue(deityForm, "PDV.Mood", 0.0)
        StorageUtil.SetIntValue(deityForm, "PDV.Mood.Band", MOOD_BAND_COOL)
    endIf
EndFunction

Float Function GetMoodAlpha(PDV_DeityBase deity)
    return JsonUtil.GetFloatValue(LIVING_DEITIES_FILE, "mood." + deity.DeityName + ".alpha", deity.MoodAlpha)
EndFunction

Int Function ComputeMoodBand(PDV_DeityBase deity, Float moodValue)
    String moodPrefix = "mood." + deity.DeityName + "."
    Float wrothMax = JsonUtil.GetFloatValue(LIVING_DEITIES_FILE, moodPrefix + "bandWrothMax", MOOD_BAND_WROTH_MAX_DEFAULT)
    Float coolMax = JsonUtil.GetFloatValue(LIVING_DEITIES_FILE, moodPrefix + "bandCoolMax", MOOD_BAND_COOL_MAX_DEFAULT)
    Float pleasedMax = JsonUtil.GetFloatValue(LIVING_DEITIES_FILE, moodPrefix + "bandPleasedMax", MOOD_BAND_PLEASED_MAX_DEFAULT)

    if moodValue < wrothMax
        return MOOD_BAND_WROTH
    elseIf moodValue < coolMax
        return MOOD_BAND_COOL
    elseIf moodValue < pleasedMax
        return MOOD_BAND_PLEASED
    endIf
    return MOOD_BAND_EXALTED
EndFunction

; Stance ceiling (M2 locked): FOREIGN -> max Pleased; TABOO/HOSTILE -> max Cool;
; lifted while the deity is the active patron (commitment) or its curse gate is
; open (Hircine face reports NATIVE-equivalence through IsRaceNativeForPlayer).
Int Function GetMoodBandCeiling(PDV_DeityBase deity)
    if deity == _activeDeity
        return MOOD_BAND_EXALTED
    endIf

    if deity.IsRaceNativeForPlayer()
        return MOOD_BAND_EXALTED
    endIf

    Int stance = deity.GetStanceForPlayer()
    if stance == deity.STANCE_TABOO || stance == deity.STANCE_HOSTILE
        return MOOD_BAND_COOL
    endIf
    return MOOD_BAND_PLEASED
EndFunction

; Active patron pool (Hades borrow): the patron, deities with a recent
; commitment-signal day, and curse-open faces. Non-pool deities keep their
; mood bookkeeping but stay silent (no omens, no demands).
Bool Function IsDeityInActivePool(PDV_DeityBase deity)
    if !deity
        return False
    endIf
    ...
    if HasRecentCommitmentSignalDays(deity, 1, 7)
        return True
    endIf

    if PDV_HircineDeity && deity == (PDV_HircineDeity as PDV_DeityBase)
        return PDV_HircineDeity.IsCurseGateOpen()
    endIf

    return False
EndFunction

; Folded into the RunDawnConsolidateScratch loop so clampedToday is read before
; PietyToday is zeroed. EWMA implemented symbolically against
; PIETY_DAILY_MAX_DELTA (never a literal) so it tracks any future retune.
Function RunDawnUpdateMoodForDeity(PDV_DeityBase deity, Float clampedToday)
    EnsureMoodState(deity)
    Form deityForm = deity as Form

    Float alphaValue = GetMoodAlpha(deity)
    Float dailyContribution = clampedToday / PIETY_DAILY_MAX_DELTA * 100.0
    Float oldMood = StorageUtil.GetFloatValue(deityForm, "PDV.Mood")
    Float newMood = ClampValue(alphaValue * dailyContribution + (1.0 - alphaValue) * oldMood, -100.0, 100.0)
    StorageUtil.SetFloatValue(deityForm, "PDV.Mood", newMood)

    Int oldBand = StorageUtil.GetIntValue(deityForm, "PDV.Mood.Band")
    Int newBand = ComputeMoodBand(deity, newMood)
    Int ceilingBand = GetMoodBandCeiling(deity)
    if newBand > ceilingBand
        newBand = ceilingBand
    endIf
...
Function OnMoodBandCross(PDV_DeityBase deity, Int oldBand, Int newBand)
    Form deityForm = deity as Form

    if newBand < oldBand
        ; Arm the dawn demand pass: a down-cross is the LD-P1 offer trigger.
        StorageUtil.SetIntValue(deityForm, "PDV.Mood.DownCrossPending", 1)
    endIf

    if deity == _activeDeity
        deity.SyncPatronBoonsToBand(GetStoredTierForDeity(deity), newBand)
        UpdatePatronMoodBandGlobal()
    endIf
    ...
    if newBand > oldBand
        SendPrismaEventToast("mood_up", deity, GetMoodBandLabel(newBand), "", "")
        Debug.Notification(deity.DeityName + "'s regard warms toward you.")
    else
        SendPrismaEventToast("mood_down", deity, GetMoodBandLabel(newBand), "", "")
        Debug.Notification(deity.DeityName + "'s regard cools.")
    endIf

    Trace(1, "Mood band cross: " + deity.DeityName + " " + GetMoodBandLabel(oldBand) + " -> " + GetMoodBandLabel(newBand))
EndFunction

Int Function GetStoredTierForDeity(PDV_DeityBase deity)
    if !deity
        return TIER_NONE
    endIf
    return StorageUtil.GetFloatValue(deity as Form, "PDV.Tier") as Int
EndFunction

Function UpdatePatronMoodBandGlobal()
    if !PDV_GLO_PatronMoodBand
        return
    endIf

    Int bandValue = MOOD_BAND_COOL
    if _activeDeity
        bandValue = GetDeityMoodBand(_activeDeity)
    endIf
    PDV_GLO_PatronMoodBand.SetValue(bandValue as Float)
EndFunction

; --- Demand scheduler (A1): mirror of RunDawnProcessCommitmentOffers ---

Function RunDawnProcessDemands()
    if !PDV_FLST_AllDeities || !JsonUtil.JsonExists(LIVING_DEITIES_FILE)
        return
    endIf

    Float nowTime = Utility.GetCurrentGameTime()
    PDV_DeityBase bestCandidate = None
    Float bestWeight = 0.0
    ...
    while i < count
        ...
        if deity
            Form deityForm = deity as Form
            if StorageUtil.GetIntValue(deityForm, "PDV.Demand.Pending") == 1
                if nowTime > StorageUtil.GetFloatValue(deityForm, "PDV.Demand.ExpiresAt")
                    ApplyDemandExpiry(deity)
                else
                    anyPending = True
                endIf
            elseIf IsEligibleForDemandOffer(deity, nowTime)
                Float weight = 0.0 - GetDeityMood(deity)
                if !bestCandidate || weight > bestWeight
                    bestCandidate = deity
                    bestWeight = weight
                endIf
            endIf
    ...
    ; LD-P1 keeps a single active demand across the roster (commitment mirror).
    if !anyPending && bestCandidate
        OfferDemand(bestCandidate, nowTime)
    endIf
EndFunction

Bool Function IsEligibleForDemandOffer(PDV_DeityBase deity, Float nowTime)
    String demandKey = JsonUtil.GetStringValue(LIVING_DEITIES_FILE, "demandKey." + deity.DeityName, "")
    if demandKey == ""
        return False
    endIf

    Form deityForm = deity as Form
    if nowTime < StorageUtil.GetFloatValue(deityForm, "PDV.Demand.CooldownUntil")
        return False
    endIf
    ...
    ; Offer trigger: a recorded down-cross while at or below Cool (M4 3.5;
    ; expectation-red joins in LD-P2).
    if StorageUtil.GetIntValue(deityForm, "PDV.Mood.DownCrossPending") != 1
        return False
    endIf

    return GetDeityMoodBand(deity) <= MOOD_BAND_COOL
EndFunction

Function OfferDemand(PDV_DeityBase deity, Float nowTime)
    Form deityForm = deity as Form
    String demandKey = JsonUtil.GetStringValue(LIVING_DEITIES_FILE, "demandKey." + deity.DeityName, "")
    if demandKey == ""
        return
    ...
    String demandPrefix = "demand." + demandKey + "."
    Int windowDays = JsonUtil.GetIntValue(LIVING_DEITIES_FILE, demandPrefix + "windowDays", 3)

    StorageUtil.SetIntValue(deityForm, "PDV.Demand.Pending", 1)
    StorageUtil.SetIntValue(deityForm, "PDV.Demand.Fulfilled", 0)
    StorageUtil.SetStringValue(deityForm, "PDV.Demand.Key", demandKey)
    StorageUtil.SetFloatValue(deityForm, "PDV.Demand.OfferedAt", nowTime)
    StorageUtil.SetFloatValue(deityForm, "PDV.Demand.ExpiresAt", nowTime + (windowDays as Float))
    ; Cache the signal binding so the hot fulfillment paths read StorageUtil only.
    StorageUtil.SetStringValue(deityForm, "PDV.Demand.EventTypesCsv", JsonUtil.GetStringValue(LIVING_DEITIES_FILE, demandPrefix + "eventTypesCsv", ""))
    StorageUtil.SetStringValue(deityForm, "PDV.Demand.EventFilter", JsonUtil.GetStringValue(LIVING_DEITIES_FILE, demandPrefix + "eventFilter", ""))
    StorageUtil.SetStringValue(deityForm, "PDV.Demand.QuestMatrixTag", JsonUtil.GetStringValue(LIVING_DEITIES_FILE, demandPrefix + "questMatrixTag", ""))
    StorageUtil.SetIntValue(deityForm, "PDV.Mood.DownCrossPending", 0)

    String demandLabel = JsonUtil.GetStringValue(LIVING_DEITIES_FILE, demandPrefix + "label", "an unnamed observance")
    SendPrismaEventToast("demand", deity, demandLabel, "", "")
    Debug.Notification(deity.DeityName + " asks something of you: " + demandLabel + ".")
    Trace(1, "Demand offered: " + demandKey + " (window " + windowDays + "d)")
EndFunction

Function ApplyDemandExpiry(PDV_DeityBase deity)
    Form deityForm = deity as Form
    String demandKey = StorageUtil.GetStringValue(deityForm, "PDV.Demand.Key")
    ...
    if PDV_HircineDeity && deity == (PDV_HircineDeity as PDV_DeityBase) && PDV_HircinePath
        PDV_HircinePath.AddStigma(1.0, "demand_expired")
    endIf

    SendPrismaEventToast("demand", deity, "unanswered", "", "")
    Debug.Notification(deity.DeityName + "'s request goes unanswered.")
    Trace(1, "Demand expired: " + demandKey)
EndFunction

; Faucet-binding fulfillment. Called by PDV_EventBus.RouteActionWithAttribution
; after a positive ScoreAction award - the existing routing point that knows
; the Int eventType (M4 3.5; no act-tag is threaded through AwardPiety).
Function NotifyDemandFaucetSignal(PDV_DeityBase deity, Int eventType, Form targetRef)
    if !deity
        return
    endIf
    ...
    String eventTypesCsv = StorageUtil.GetStringValue(deityForm, "PDV.Demand.EventTypesCsv")
    if eventTypesCsv == ""
        return
    endIf

    Bool matched = False
    String[] eventTokens = StringUtil.Split(eventTypesCsv, "|")
    Int i = 0
    while i < eventTokens.Length && !matched
        if (eventTokens[i] as Int) == eventType
            matched = True
        endIf
        i += 1
    endWhile
    ...
    String eventFilter = StorageUtil.GetStringValue(deityForm, "PDV.Demand.EventFilter")
    if eventFilter != "" && !IsDemandEventFilterSatisfied(eventFilter, targetRef)
        return
    endIf
...
; Quest-matrix-binding fulfillment. Called from ApplyDeityReaction after a
; positive matrix award; keyed by deity NAME so the Hircine path's matrix rows
; can fulfill the deity face's demand without re-routing the piety itself.
Function NotifyDemandQuestTag(String deityName, String sourceTag)
    if sourceTag == ""
        return
    endIf

    PDV_DeityBase demandDeity = GetDeityByName(deityName)
    if !demandDeity
        return
    endIf

    Form deityForm = demandDeity as Form
    if StorageUtil.GetIntValue(deityForm, "PDV.Demand.Pending") != 1
        return
    endIf

    if StorageUtil.GetStringValue(deityForm, "PDV.Demand.QuestMatrixTag") != sourceTag
        return
    endIf

    FulfillDemand(demandDeity)
EndFunction

Bool Function IsDemandEventFilterSatisfied(String eventFilter, Form targetRef)
    if eventFilter == "great_beast"
        if !PDV_FLST_DemandGreatBeasts || !targetRef
            return False
        endIf
        Actor victim = targetRef as Actor
        if !victim
            return False
        endIf
        ...
        if !victimRace
            return False
        endIf
        return PDV_FLST_DemandGreatBeasts.HasForm(victimRace)
    endIf

    ; Unknown filter vocab: fail closed (honest no-op) rather than over-credit.
    return False
EndFunction

; Single-act reset (Sacrosanct feeding borrow): fulfillment clears the demand
; outright and uplifts mood once.
Function FulfillDemand(PDV_DeityBase deity)
    Form deityForm = deity as Form
    String demandKey = StorageUtil.GetStringValue(deityForm, "PDV.Demand.Key")
...
; Recomputes the band through the same ceiling + crossing dispatch.
Function ApplyMoodDelta(PDV_DeityBase deity, Float delta, String reason)
    EnsureMoodState(deity)
    Form deityForm = deity as Form

    Float newMood = ClampValue(StorageUtil.GetFloatValue(deityForm, "PDV.Mood") + delta, -100.0, 100.0)
    StorageUtil.SetFloatValue(deityForm, "PDV.Mood", newMood)
    ...
    Trace(2, "Mood delta " + delta + " (" + reason + ") -> " + deity.DeityName + " " + newMood)
EndFunction

; --- Dream omens (A2): rides OnSleepStart via PDV_EventBus ---

Function HandleSleepDreamOmen()
    if !PDV_FLST_AllDeities || !JsonUtil.JsonExists(LIVING_DEITIES_FILE)
        return
    endIf

    ; Day-keyed cooldown (encoded +1 so day 0 is distinguishable from unset).
    Int currentDay = (Utility.GetCurrentGameTime() - 0.25) as Int
    Int lastDreamDayEncoded = StorageUtil.GetIntValue(None, "PDV.Omen.LastDreamDay")
    if lastDreamDayEncoded > 0 && (currentDay - (lastDreamDayEncoded - 1)) < (DREAM_OMEN_COOLDOWN_DAYS as Int)
        return
    endIf

    ; Pick the first pool deity with an armed dream (band-crossed recently)
    ; sitting at an extreme band. Pool size stays small (<= ~4), so first-match
    ; is acceptable for LD-P1.
    PDV_DeityBase dreamDeity = None
    Int i = 0
    Int count = PDV_FLST_AllDeities.GetSize()
    while i < count && !dreamDeity
        PDV_DeityBase deity = PDV_FLST_AllDeities.GetAt(i) as PDV_DeityBase
        if deity && StorageUtil.GetIntValue(deity as Form, "PDV.Omen.DreamArmed") == 1 && IsDeityInActivePool(deity)
            Int bandValue = GetDeityMoodBand(deity)
            if bandValue == MOOD_BAND_WROTH || bandValue == MOOD_BAND_EXALTED
                dreamDeity = deity
            endIf
        endIf
        i += 1
    ...
    if Utility.RandomFloat(0.0, 1.0) > DREAM_OMEN_CHANCE
        return
    endIf

    StorageUtil.SetIntValue(None, "PDV.Omen.LastDreamDay", currentDay + 1)
    StorageUtil.SetIntValue(dreamDeity as Form, "PDV.Omen.DreamArmed", 0)

    if !SendPrismaEventToast("dream", dreamDeity, GetMoodBandLabel(GetDeityMoodBand(dreamDeity)), "", "")
        Debug.Notification("Your sleep is touched by " + dreamDeity.DeityName + ".")
    endIf
    Trace(1, "Dream omen fired: " + dreamDeity.DeityName)
EndFunction
