...
; EWMA weight for the dawn mood recompute in PDV__ManagerQuest. Authoring source
; is references/authoring/PDV_DeityMood.csv (Kyne 0.12, Hircine 0.22); the JSON
; value wins when present so saves track retunes without CK property migration.
Float Property MoodAlpha = 0.15 Auto
; Band-indexed boon variants layered on top of the cumulative tier boons while
; this deity is the active patron. Only Pleased/Exalted have variants; Wroth and
; Cool fall back to the base tier boons. Unwired (None) variants are skipped.
Spell Property Boon_Seeker_Pleased Auto
Spell Property Boon_Seeker_Exalted Auto
Spell Property Boon_Devoted_Pleased Auto
Spell Property Boon_Devoted_Exalted Auto
Spell Property Boon_Champion_Pleased Auto
Spell Property Boon_Champion_Exalted Auto
; Deity-themed clutch-save ability (PDV_T3DailyLowHealthSaveEffect carrier).
Spell Property ClutchSaveEffect Auto
...
Function ClearAllBoons()
    ...
    endIf
    ClearBandBoonVariants()
EndFunction

; --- LD-P1 Living Deities (research slice) ---

Function ClearBandBoonVariants()
    if Boon_Seeker_Pleased
        Game.GetPlayer().RemoveSpell(Boon_Seeker_Pleased)
    endIf
    if Boon_Seeker_Exalted
        Game.GetPlayer().RemoveSpell(Boon_Seeker_Exalted)
    endIf
    if Boon_Devoted_Pleased
        Game.GetPlayer().RemoveSpell(Boon_Devoted_Pleased)
    endIf
    if Boon_Devoted_Exalted
        Game.GetPlayer().RemoveSpell(Boon_Devoted_Exalted)
    endIf
    if Boon_Champion_Pleased
        Game.GetPlayer().RemoveSpell(Boon_Champion_Pleased)
    endIf
    if Boon_Champion_Exalted
        Game.GetPlayer().RemoveSpell(Boon_Champion_Exalted)
    endIf
EndFunction

; Band-aware boon sync. Mirrors SyncPatronBoonsToTier exactly (clear-before-add,
; one-active-boost discipline) and then layers the band variant of the CURRENT
; tier on top. Re-granted only on tier or band transitions, never per-frame, so
; the applied-MGEF-cannot-reread-a-global engine limitation does not apply.
Function SyncPatronBoonsToBand(Int tierValue, Int bandValue)
    SyncPatronBoonsToTier(tierValue)

    Spell bandBoon = GetBandBoonVariant(tierValue, bandValue)
    if bandBoon
        Game.GetPlayer().AddSpell(bandBoon, False)
    endIf
EndFunction

Spell Function GetBandBoonVariant(Int tierValue, Int bandValue)
    if bandValue == MOOD_BAND_PLEASED
        if tierValue == TIER_SEEKER
            return Boon_Seeker_Pleased
        elseIf tierValue == TIER_DEVOTED
            return Boon_Devoted_Pleased
        elseIf tierValue == TIER_CHAMPION
            return Boon_Champion_Pleased
        endIf
    elseIf bandValue == MOOD_BAND_EXALTED
        if tierValue == TIER_SEEKER
            return Boon_Seeker_Exalted
        elseIf tierValue == TIER_DEVOTED
            return Boon_Devoted_Exalted
        elseIf tierValue == TIER_CHAMPION
            return Boon_Champion_Exalted
        endIf
    endIf
