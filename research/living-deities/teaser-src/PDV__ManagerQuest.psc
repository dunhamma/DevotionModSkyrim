...
; --- LD-P1 Mood Teaser (research slice) ---
; Active patron's mood band mirror for CK Condition / MGEF reads (script-poll
; reads only; refreshed at dawn + on band-cross). 0=Wroth 1=Cool 2=Pleased 3=Exalted.
...
; --- LD-P1 Mood Teaser (research slice) ---
; Optional runtime tunables JSON (per-deity alpha + band thresholds), compiled by
; tools/pdv_living_deities_compile.mjs. Absent = the property/constant defaults
; below are used (the mood path degrades cleanly with no JSON).
...
Function RunDawnConsolidateScratch()
    ...
    while i < count
        ...
        if deity
            ...
            ; LD-P1 Mood Teaser: EWMA folded into this loop so it reads clampedToday
...
;/ =====================================================================
    LD-P1 Mood Teaser (research slice; net-new V1 candidate)
    ---------------------------------------------------------------------
    Per-deity mood EWMA over the daily clampedToday, 4 asymmetric bands, and a
    once-per-day band-cross toast. This is the mood subset of the full LD-P1
    engine with NO demands, band-variant boons, dream omens, clutch save, or
    great-beast filter. Tunables ride the optional JSON (LIVING_DEITIES_FILE);
    absent = property/constant defaults. Spec: references/research/living-
    deities/04_living_deities_architecture.md.
   ===================================================================== /;
...
; lifted while the deity is the active patron or native to the player's race.
...
; commitment-signal day. Non-pool deities keep their mood bookkeeping but stay
; silent (no toast). Teaser drops the Hircine-curse branch of the full engine.
...
; Teaser band-cross handler: mood-band mirror + once-per-day toast only. The full
; engine's demand-arming and band-boon swap are intentionally NOT present here.
