    ...
    AuthorReport report)
{
    ...
    {
        ObjectProp("PDV_Bless_Orc_Spine_City", RequireRecord<Spell>(index, "PDV_Bless_Orc_Spine_City").FormKey),
        ObjectProp("PDV_Bless_Orc_Spine_Stronghold", RequireRecord<Spell>(index, "PDV_Bless_Orc_Spine_Stronghold").FormKey),
        ObjectProp("PDV_Bless_Orc_Spine_LegionExile", RequireRecord<Spell>(index, "PDV_Bless_Orc_Spine_LegionExile").FormKey),
    });
    report.Actions.Add("Wired Orc spine boon properties on PDV__ManagerQuest.");
}
    ...
    AuthorReport report)
{
    ...
    if (manager is not null)
    {
        ...
        {
            CheckObjectProperty(script, "PDV_Bless_Orc_Spine_City", RequireRecord<Spell>(index, "PDV_Bless_Orc_Spine_City").FormKey, managerEdid, report);
            CheckObjectProperty(script, "PDV_Bless_Orc_Spine_Stronghold", RequireRecord<Spell>(index, "PDV_Bless_Orc_Spine_Stronghold").FormKey, managerEdid, report);
            CheckObjectProperty(script, "PDV_Bless_Orc_Spine_LegionExile", RequireRecord<Spell>(index, "PDV_Bless_Orc_Spine_LegionExile").FormKey, managerEdid, report);
        }
    }
}
static void CheckSpell(Dictionary<string, ISkyrimMajorRecordGetter> index, SpellSpec spec, AuthorReport report)
{
    ...
    {
        report.Errors.Add($"{spec.SpellEditorId} description does not match the Orc spine contract.");
    }
...
static void CheckMagicEffect(MagicEffect mgef, SpellSpec spellSpec, EffectSpec effectSpec, AuthorReport report)
{
    ...
    {
        report.Errors.Add($"{effectSpec.EffectEditorId} description does not match the Orc spine contract.");
    }
...
static void WriteMod(SkyrimMod mod, string espPath, AuthorReport report)
{
    ...
    var backupDir = Path.Combine(Path.GetDirectoryName(Path.GetFullPath(espPath))!, "Backups", "orc-spine");
    ...
    var tempPath = $"{espPath}.orc-spine.tmp";
...
[
    new(
        "PDV_Bless_Orc_Spine_City",
        "Orc Spine - City",
        "The code holds under stone and strangers. Armor +5, Maximum Health +10.",
        [
            new("PDV_MGEF_Orc_Spine_City_Armor", "Orc City Spine Armor", "DamageResist", 5.0f),
            new("PDV_MGEF_Orc_Spine_City_Health", "Orc City Spine Health", "Health", 10.0f),
        ]),
    new(
        "PDV_Bless_Orc_Spine_Stronghold",
        "Orc Spine - Stronghold",
        "The code holds where forge and kin stand near. Armor +8, Maximum Health +10.",
        [
            new("PDV_MGEF_Orc_Spine_Stronghold_Armor", "Orc Stronghold Spine Armor", "DamageResist", 8.0f),
            new("PDV_MGEF_Orc_Spine_Stronghold_Health", "Orc Stronghold Spine Health", "Health", 10.0f),
        ]),
    new(
        "PDV_Bless_Orc_Spine_LegionExile",
        "Orc Spine - Legion-Exile",
        "The code holds under foreign order and exile. Armor +5, Maximum Health +15.",
        [
            new("PDV_MGEF_Orc_Spine_LegionExile_Armor", "Orc Legion-Exile Spine Armor", "DamageResist", 5.0f),
            new("PDV_MGEF_Orc_Spine_LegionExile_Health", "Orc Legion-Exile Spine Health", "Health", 15.0f),
        ]),
];
