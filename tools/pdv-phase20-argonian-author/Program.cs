...
const string defaultManifest = @"references\authoring\PDV_Phase20ArgonianImplementationCosting.manifest.json";
const string argonianSubstrate = "PDV_Substrate_ArgonianHist";
const string argonianPostureTrack = "PDV_State_ArgonianHistPosture";
...
try
{
    ...
    {
        throw new FileNotFoundException("Argonian implementation manifest not found.", manifestPath);
    }

    var manifest = LoadManifest(manifestPath);
    var postureLabels = GetStateLabels(manifest, argonianPostureTrack);
    ...
    if (checkPlacements)
    {
        ...
        {
            throw new InvalidOperationException("Argonian placement check failed.");
        }
    ...
    else
    {
        ...
        var substrate = createMissing
            ? EnsureQuest(mod, index, allocator, argonianSubstrate, report)
            : RequireRecord<Quest>(index, argonianSubstrate);
        ConfigureQuestShell(substrate, argonianSubstrate);
        WireQuestScript(substrate, "PDV_Substrate_ArgonianHist", new ScriptProperty[]
        {
            StringProp("SubstrateName", "ArgonianHist"),
            IntProp("RequiredOriginRace", 7),
            ObjectProp("PDV_GLO_OriginRace", originGlobal.FormKey),
            ObjectProp("PDV_GLO_DebugLevel", debugGlobal.FormKey),
        });
        report.Actions.Add("Ensured PDV_Substrate_ArgonianHist concrete script wiring.");

        var postureTrack = createMissing
            ? EnsureQuest(mod, index, allocator, argonianPostureTrack, report)
            : RequireRecord<Quest>(index, argonianPostureTrack);
        ConfigureQuestShell(postureTrack, argonianPostureTrack);
        WireQuestScript(postureTrack, "PDV_StateTrack", new ScriptProperty[]
        {
            StringProp("TrackName", "ArgonianHistPosture"),
            ObjectProp("PDV_GLO_DebugLevel", debugGlobal.FormKey),
            StringListProp("StateLabels", postureLabels),
        });
        report.Actions.Add("Ensured PDV_State_ArgonianHistPosture is wired to PDV_StateTrack.");
        ...
        {
            ObjectProp("PDV_ArgonianHistSubstrate", substrate.FormKey),
            ObjectProp("PDV_ArgonianHistPostureTrack", postureTrack.FormKey),
        });
        report.Actions.Add("Wired Argonian substrate and posture properties on PDV__ManagerQuest.");

        WriteModIfNeeded(mod, espPath, dryRun, report, "phase20-argonian");
        report.Status = "PASS";
    }
}
catch (Exception ex)
...
static ArgonianManifest LoadManifest(string manifestPath)
{
    var parsed = JsonSerializer.Deserialize<ArgonianManifest>(File.ReadAllText(manifestPath));
    if (parsed is null)
    {
        throw new InvalidOperationException("Argonian implementation manifest could not be parsed.");
    }

    if (parsed.stateSurfaces is null || parsed.stateSurfaces.Count == 0)
    {
        throw new InvalidOperationException("Argonian implementation manifest has no state surfaces.");
    }

    return parsed;
}

static string[] GetStateLabels(ArgonianManifest manifest, string editorId)
{
    ...
    {
        throw new InvalidOperationException($"Argonian implementation manifest is missing {editorId} enum.");
    }
    ...
    if (ordered.Length != 5 || ordered[0] != "Normal" || ordered[4] != "Corrupted")
    {
        throw new InvalidOperationException($"{editorId} enum does not match the locked Argonian posture contract.");
    }

    return ordered;
}

static TriggerDefinition[] BuildTriggerDefinitions(ArgonianManifest manifest)
{
    ...
    foreach (var trigger in manifest.triggerSurfaces ?? [])
    {
        ...
        {
            throw new InvalidOperationException($"Argonian trigger surface {trigger.id ?? "(unknown)"} must be an ACTI record.");
        }

        if (string.IsNullOrWhiteSpace(trigger.editorId)
            || trigger.routeId <= 0
            || trigger.requiredOriginRace != 7
        ...
        {
            throw new InvalidOperationException($"Argonian trigger surface metadata is incomplete for {trigger.id ?? "(unknown)"}.");
        }
