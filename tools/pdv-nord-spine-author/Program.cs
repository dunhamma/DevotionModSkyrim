...
const string substrateEdid = "PDV_Substrate_NordAncestor";
    ...
    AuthorReport report)
{
    ...
    ConfigureQuestShell(substrate, "Nord Ancestor Spine");
    var substrateProperties = new ScriptProperty[]
    {
        StringProp("SubstrateName", "NordAncestor"),
        IntProp("RequiredOriginRace", 0),
        ObjectProp("PDV_GLO_OriginRace", originGlobal.FormKey),
        ObjectProp("PDV_GLO_DebugLevel", debugGlobal.FormKey),
        ObjectProp("Substrate_Always", RequireRecord<Spell>(index, "PDV_Bless_Nord_Substrate_Always").FormKey),
        ObjectProp("Substrate_Mid", RequireRecord<Spell>(index, "PDV_Bless_Nord_Substrate_Mid").FormKey),
        ObjectProp("Substrate_High", RequireRecord<Spell>(index, "PDV_Bless_Nord_Substrate_High").FormKey),
    };
    WireQuestScript(substrate, "PDV_SubstrateBase", substrateProperties);
    WireQuestScript(substrate, "PDV_Substrate_NordAncestor", substrateProperties);
    report.Actions.Add("Wired PDV_Substrate_NordAncestor base and concrete scripts.");

    var manager = RequireRecord<Quest>(index, managerEdid);
    WireQuestScript(manager, "PDV__ManagerQuest", new ScriptProperty[]
    {
        ObjectProp("PDV_NordAncestorSubstrate", substrate.FormKey),
    });
    report.Actions.Add("Wired PDV_NordAncestorSubstrate on PDV__ManagerQuest.");
    ...
    AuthorReport report)
{
    ...
    if (substrate is not null)
    {
        ...
        var concrete = CheckQuestScript(substrate, "PDV_Substrate_NordAncestor", report);
        if (concrete is not null)
        {
            CheckStringProperty(concrete, "SubstrateName", "NordAncestor", substrateEdid, report);
            CheckIntProperty(concrete, "RequiredOriginRace", 0, substrateEdid, report);
            ...
            CheckObjectProperty(concrete, "Substrate_Always", RequireRecord<Spell>(index, "PDV_Bless_Nord_Substrate_Always").FormKey, substrateEdid, report);
            CheckObjectProperty(concrete, "Substrate_Mid", RequireRecord<Spell>(index, "PDV_Bless_Nord_Substrate_Mid").FormKey, substrateEdid, report);
            CheckObjectProperty(concrete, "Substrate_High", RequireRecord<Spell>(index, "PDV_Bless_Nord_Substrate_High").FormKey, substrateEdid, report);
        }
    }
    ...
    if (manager is not null)
    {
        ...
        {
            CheckObjectProperty(script, "PDV_NordAncestorSubstrate", RequireRecord<Quest>(index, substrateEdid).FormKey, managerEdid, report);
        }
    }
...
static void CheckSpell(Dictionary<string, ISkyrimMajorRecordGetter> index, SpellSpec spec, AuthorReport report)
{
    ...
    {
        report.Errors.Add($"{spec.SpellEditorId} description does not match the Nord substrate contract.");
    }
...
static void CheckMagicEffect(MagicEffect mgef, SpellSpec spellSpec, EffectSpec effectSpec, AuthorReport report)
{
    ...
    {
        report.Errors.Add($"{effectSpec.EffectEditorId} description does not match the Nord substrate contract.");
    }
...
static void WriteMod(SkyrimMod mod, string espPath, AuthorReport report)
{
    ...
    var backupDir = Path.Combine(Path.GetDirectoryName(Path.GetFullPath(espPath))!, "Backups", "nord-spine");
    ...
    var tempPath = $"{espPath}.nord-spine.tmp";
...
[
    new(
        "PDV_Bless_Nord_Substrate_Always",
        "Nord Ancestor's Steadiness",
        "The old line steadies you against cold and hardship. Frost Resistance +5%, Health +10.",
        [
            new("PDV_MGEF_Nord_Substrate_Always_Frost", "Ancestor's Frost Steadiness", "ResistFrost", 5.0f),
            new("PDV_MGEF_Nord_Substrate_Always_Health", "Ancestor's Hardiness", "Health", 10.0f),
        ]),
    new(
        "PDV_Bless_Nord_Substrate_Mid",
        "Nord Ancestor's Regard",
        "The old line stands closer in you. Frost Resistance +15%, Health +25.",
        [
            new("PDV_MGEF_Nord_Substrate_Mid_Frost", "Ancestor's Frost Regard", "ResistFrost", 15.0f),
            new("PDV_MGEF_Nord_Substrate_Mid_Health", "Ancestor's Regard", "Health", 25.0f),
        ]),
    new(
        "PDV_Bless_Nord_Substrate_High",
        "Nord Ancestor's Honor",
        "The old line stands strong in you. Frost Resistance +30%, Health +50.",
        [
            new("PDV_MGEF_Nord_Substrate_High_Frost", "Ancestor's Frost Honor", "ResistFrost", 30.0f),
            new("PDV_MGEF_Nord_Substrate_High_Health", "Ancestor's Honor", "Health", 50.0f),
        ]),
];
