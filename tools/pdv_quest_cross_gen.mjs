...
function loadTranches() {
  ...
  for (const f of files) {
    const rows = parseCsv(readFileSync(path.join(AUTHORING, f), "utf8"));
    ...
    for (const r of rows.slice(1)) {
      ...
      if (!outcomes.has(oKey)) outcomes.set(oKey, { editorId, questName, stage, outcome, tags: new Set() });
      const entry = outcomes.get(oKey);
      for (const t of tags) entry.tags.add(t);
      if (!entry.outcome && outcome) entry.outcome = outcome;
      existingPairs.add(deity + "|" + oKey);
    }
  }
...
function main() {
  ...
  const { outcomes, existingPairs } = loadTranches();
  const { candidates, conflicts } = crossGenerate(profiles, outcomes, existingPairs);

  mkdirSync(OUT_DIR, { recursive: true });
  const header = "editor_id,quest_name,outcome_stage,outcome,act_tags,deity,valence,intensity,magnitude,citation";
  const candCsv = [header, ...candidates.map(c => [c.editorId, c.questName, c.stage, c.outcome, c.tag, c.deity, c.valence, c.intensity, c.magnitude, c.citation].map(csvEscape).join(","))].join("\n") + "\n";
  writeFileSync(path.join(OUT_DIR, "PDV_QuestCrossGen_Candidates.csv"), candCsv);
  ...
  writeFileSync(path.join(OUT_DIR, "PDV_QuestCrossGen_Conflicts.csv"), confCsv);
  ...
  writeFileSync(path.join(OUT_DIR, "PDV_QuestCrossGen_Summary.md"), md);
