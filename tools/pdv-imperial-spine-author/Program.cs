...
const string substrateEdid = "PDV_Substrate_ImperialAncestor";
...
static void AuthorImperialSpine(
    SkyrimMod mod,
    ...
    AuthorReport report)
{
    ...
    ConfigureQuestShell(substrate, "Imperial Ancestor Spine");
    var substrateProperties = new ScriptProperty[]
    {
        StringProp("SubstrateName", "ImperialAncestor"),
        ...
        ObjectProp("Substrate_Always", RequireRecord<Spell>(index, "PDV_Bless_Imperial_Spine_Always").FormKey),
        ObjectProp("Substrate_Mid", RequireRecord<Spell>(index, "PDV_Bless_Imperial_Spine_Mid").FormKey),
        ObjectProp("Substrate_High", RequireRecord<Spell>(index, "PDV_Bless_Imperial_Spine_High").FormKey),
    };
    WireQuestScript(substrate, "PDV_SubstrateBase", substrateProperties);
    WireQuestScript(substrate, "PDV_Substrate_ImperialAncestor", substrateProperties);
    report.Actions.Add("Wired PDV_Substrate_ImperialAncestor base and concrete scripts.");

    var manager = RequireRecord<Quest>(index, managerEdid);
    WireQuestScript(manager, "PDV__ManagerQuest", new ScriptProperty[]
    {
        ObjectProp("PDV_ImperialAncestorSubstrate", substrate.FormKey),
    });
    report.Actions.Add("Wired PDV_ImperialAncestorSubstrate on PDV__ManagerQuest.");
...
static void CheckImperialSpine(
    Dictionary<string, ISkyrimMajorRecordGetter> index,
    IReadOnlyList<SpellSpec> spellSpecs,
    AuthorReport report)
{
    ...
    if (substrate is not null)
    {
        ...
        var concrete = CheckQuestScript(substrate, "PDV_Substrate_ImperialAncestor", report);
        if (concrete is not null)
        {
            CheckStringProperty(concrete, "SubstrateName", "ImperialAncestor", substrateEdid, report);
            CheckIntProperty(concrete, "RequiredOriginRace", 1, substrateEdid, report);
            ...
            CheckObjectProperty(concrete, "Substrate_Always", RequireRecord<Spell>(index, "PDV_Bless_Imperial_Spine_Always").FormKey, substrateEdid, report);
            CheckObjectProperty(concrete, "Substrate_Mid", RequireRecord<Spell>(index, "PDV_Bless_Imperial_Spine_Mid").FormKey, substrateEdid, report);
            CheckObjectProperty(concrete, "Substrate_High", RequireRecord<Spell>(index, "PDV_Bless_Imperial_Spine_High").FormKey, substrateEdid, report);
        }
    }
    ...
    if (manager is not null)
    {
        ...
        {
            CheckObjectProperty(script, "PDV_ImperialAncestorSubstrate", RequireRecord<Quest>(index, substrateEdid).FormKey, managerEdid, report);
        }
    }
...
static void CheckSpell(Dictionary<string, ISkyrimMajorRecordGetter> index, SpellSpec spec, AuthorReport report)
{
    ...
    {
        report.Errors.Add($"{spec.SpellEditorId} description does not match the Imperial substrate contract.");
    }
...
static void CheckMagicEffect(MagicEffect mgef, SpellSpec spellSpec, EffectSpec effectSpec, AuthorReport report)
{
    ...
    {
        report.Errors.Add($"{effectSpec.EffectEditorId} description does not match the Imperial substrate contract.");
    }
...
static void WriteMod(SkyrimMod mod, string espPath, AuthorReport report)
{
    ...
    var backupDir = Path.Combine(Path.GetDirectoryName(Path.GetFullPath(espPath))!, "Backups", "imperial-spine");
    ...
    var tempPath = $"{espPath}.imperial-spine.tmp";
...
[
    ...
    new(
        ...
        "Imperial Spine - Civic Discipline",
        "The Empire's old habits hold under pressure. Maximum Health +10, Maximum Stamina +10.",
        [
            new("PDV_MGEF_Imperial_Spine_Mid_Health", "Imperial Civic Discipline Health", "Health", 10.0f),
            new("PDV_MGEF_Imperial_Spine_Mid_Stamina", "Imperial Civic Discipline Stamina", "Stamina", 10.0f),
        ]),
    new(
        "PDV_Bless_Imperial_Spine_High",
        "Imperial Spine - Civic Exemplar",
        "The old civic spine stands firm. Maximum Health +15, Maximum Stamina +15.",
        [
            new("PDV_MGEF_Imperial_Spine_High_Health", "Imperial Civic Exemplar Health", "Health", 15.0f),
            new("PDV_MGEF_Imperial_Spine_High_Stamina", "Imperial Civic Exemplar Stamina", "Stamina", 15.0f),
        ]),
];
