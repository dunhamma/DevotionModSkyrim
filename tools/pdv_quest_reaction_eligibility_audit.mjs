...
export function evaluate(managerSource) {
  ...
  add(findings, /originRace\s*==\s*ORIGIN_NORD[\s\S]*?return\s+IsNordOfferEligibleDeity\s*\(\s*deity\s*\)/i.test(reachable), "policy.nord-selected-lane", "Nord quest reactions delegate to the selected-baseline eligibility policy.");
  ...
  add(findings, /originRace\s*==\s*ORIGIN_NORD[\s\S]*?return\s+IsNordOfferEligibleDeity\s*\(\s*deity\s*\)/i.test(genericEligible), "alignment.generic-likes-dislikes", "Generic likes/dislikes and quest reactions share Nord selected-lane policy.");
...
function runSelfTest(managerSource) {
  ...
  const broken = evaluate(managerSource.replace("return IsNordOfferEligibleDeity(deity)", "return IsDashboardDeityInOriginRoster(deity, originRace)"));
