...
const KNOWN_FLAGS = new Set(["--allow-overflow", "--expected-sequence", "--help", "--json", "--log", "--max-admission-ms", "--max-job-ms", "--self-test"]);
...
const ACTIONS = ["ENQUEUE", "COALESCE", "BUILD", "START", "COMPLETE", "RESUME", "OVERFLOW"];
...
function usage() {
  return [
    ...
    "  --max-admission-ms <milliseconds> Fail when an ENQUEUE admission exceeds the limit.",
...
function parseArgs(argv) {
  const options = { logPath: DEFAULT_LOG, expectedSequence: [], maxAdmissionMs: null, maxJobMs: null, allowOverflow: false, selfTest: false, json: false, help: false };
  for (let index = 0; index < argv.length; index += 1) {
    ...
    } else if (arg === "--max-admission-ms") {
      const raw = argv[++index];
      options.maxAdmissionMs = Number(raw);
      if (!raw || !Number.isFinite(options.maxAdmissionMs) || options.maxAdmissionMs <= 0) {
        throw new Error("--max-admission-ms requires a positive number.");
      }
...
function parseMarker(line, lineNumber) {
  const match = line.match(/\[PDV\]\[QR_QUEUE\]\s+(ENQUEUE|COALESCE|BUILD|START|COMPLETE|RESUME|OVERFLOW)\b/i);
  if (!match) return null;
  ...
  const admissionRaw = token(line, ["admissionMs"]);
  const legacyBuildRaw = token(line, ["buildMs"]);
  ...
  const implicitJob = ["ENQUEUE", "BUILD", "START", "COMPLETE"].includes(action)
  ...
  return {
    ...
    admissionMs: admissionRaw !== "" && Number.isFinite(Number(admissionRaw)) ? Number(admissionRaw) : null,
    buildMs: legacyBuildRaw !== "" && Number.isFinite(Number(legacyBuildRaw)) ? Number(legacyBuildRaw) : null,
...
export function evaluateLog(logText, options = {}) {
  ...
  const durations = pairs.map(({ enqueue, start, complete }) => {
    ...
    return { job: complete.job, key: complete.key, line: complete.line, jobMs, endToEndMs, ingressMs, admissionMs: enqueue?.admissionMs ?? null, legacyIngressBuildMs: enqueue?.buildMs ?? null };
  });
  const expected = options.expectedSequence ?? [];
  const enqueueKeys = byAction.ENQUEUE.map((entry) => entry.key).filter(Boolean);
  const buildKeys = byAction.BUILD.map((entry) => entry.key).filter(Boolean);
  ...
  add(byAction.BUILD.length > 0, "build-present", "At least one queued job completed bounded catalog materialization.");
  ...
  if (options.maxAdmissionMs != null) {
    const unavailable = byAction.ENQUEUE.filter((entry) => entry.admissionMs == null);
    const slow = byAction.ENQUEUE.filter((entry) => entry.admissionMs != null && entry.admissionMs > options.maxAdmissionMs);
    add(
      unavailable.length === 0 && slow.length === 0,
      "admission-latency-within-limit",
      unavailable.length
        ? `${unavailable.length} enqueue marker(s) lack admissionMs.`
        : (slow.length
          ? `${slow.length} admission(s) exceeded ${options.maxAdmissionMs} ms: ${slow.map((entry) => `${entry.job || entry.key}=${entry.admissionMs}`).join(", ")}.`
          : `Every queue admission finished within ${options.maxAdmissionMs} ms.`),
    );
  }
  ...
  if (expected.length) {
    ...
    add(suffixMatches(buildKeys, expected), "expected-build-sequence", `Expected final build keys: ${expected.join(" -> ")}; saw ${buildKeys.slice(-expected.length).join(" -> ") || "none"}.`);
  ...
  return {
    ...
    durations,
    sequence: { expected, enqueue: enqueueKeys, build: buildKeys, start: startKeys, complete: completeKeys },
  };
}
...
function selfTestLog() {
  return [
    ...
    "[08/14/2026 - 05:56:23PM] [PDV][QR_QUEUE] ENQUEUE queued v3qr_2 key=Skyrim.esm|210731|150 cells=0 sourceCells=45 materialized=0 admissionMs=24.000000 pending=1",
    "[08/14/2026 - 05:56:23PM] [PDV][QR_QUEUE] ENQUEUE queued v3qr_3 key=Skyrim.esm|148154|160 cells=0 sourceCells=45 materialized=0 admissionMs=18.000000 pending=2",
    "[08/14/2026 - 05:56:24PM] [PDV][QR_QUEUE] ENQUEUE queued v3qr_4 key=Skyrim.esm|207142|200 cells=0 sourceCells=45 materialized=0 admissionMs=21.000000 pending=3",
    "[08/14/2026 - 05:56:25PM] [PDV][QR_QUEUE] ENQUEUE queued v3qr_5 key=Skyrim.esm|221587|220 cells=0 sourceCells=22 materialized=0 admissionMs=16.000000 pending=4",
    "[08/14/2026 - 05:56:26PM] [PDV][QR_QUEUE] BUILD completed v3qr_2 key=Skyrim.esm|210731|150 cells=25 sourceCells=45 skipped=20 meta=0 materializeMs=3000.000000",
    "[08/14/2026 - 05:56:26PM] [PDV][QR_QUEUE] START started v3qr_2 key=Skyrim.esm|210731|150 cells=25 sourceCells=45 skipped=20 meta=0",
    ...
    "[08/14/2026 - 05:56:31PM] [PDV][QR_QUEUE] BUILD completed v3qr_3 key=Skyrim.esm|148154|160 cells=25 sourceCells=45 skipped=20 meta=0 materializeMs=8000.000000",
    ...
    "[08/14/2026 - 05:56:36PM] [PDV][QR_QUEUE] BUILD completed v3qr_4 key=Skyrim.esm|207142|200 cells=25 sourceCells=45 skipped=20 meta=0 materializeMs=12000.000000",
    ...
    "[08/14/2026 - 05:56:42PM] [PDV][QR_QUEUE] BUILD completed v3qr_5 key=Skyrim.esm|221587|220 cells=11 sourceCells=22 skipped=11 meta=0 materializeMs=17000.000000",
    ...
    "[08/14/2026 - 06:06:23PM] [PDV][QR_QUEUE] ENQUEUE queued v3qr_2 key=Skyrim.esm|210731|150 cells=0 sourceCells=45 materialized=0 admissionMs=20.000000 pending=1",
    "[08/14/2026 - 06:06:23PM] [PDV][QR_QUEUE] BUILD completed v3qr_2 key=Skyrim.esm|210731|150 cells=21 sourceCells=45 skipped=24 meta=0 materializeMs=4000.000000",
    ...
    "[08/14/2026 - 06:06:23PM] [PDV][QR_QUEUE] ENQUEUE queued v3qr_3 key=Skyrim.esm|148154|160 cells=0 sourceCells=45 materialized=0 admissionMs=17.000000 pending=2",
    "[08/14/2026 - 06:06:24PM] [PDV][QR_QUEUE] ENQUEUE queued v3qr_4 key=Skyrim.esm|207142|200 cells=0 sourceCells=45 materialized=0 admissionMs=19.000000 pending=3",
    "[08/14/2026 - 06:06:25PM] [PDV][QR_QUEUE] ENQUEUE queued v3qr_5 key=Skyrim.esm|221587|220 cells=0 sourceCells=22 materialized=0 admissionMs=15.000000 pending=4",
    ...
    "[08/14/2026 - 06:06:27PM] [PDV][QR_QUEUE] BUILD completed v3qr_3 key=Skyrim.esm|148154|160 cells=21 sourceCells=45 skipped=24 meta=0 materializeMs=4000.000000",
    ...
    "[08/14/2026 - 06:06:31PM] [PDV][QR_QUEUE] BUILD completed v3qr_4 key=Skyrim.esm|207142|200 cells=21 sourceCells=45 skipped=24 meta=0 materializeMs=7000.000000",
    ...
    "[08/14/2026 - 06:06:35PM] [PDV][QR_QUEUE] BUILD completed v3qr_5 key=Skyrim.esm|221587|220 cells=7 sourceCells=22 skipped=15 meta=0 materializeMs=10000.000000",
...
function printReport(report) {
  ...
  for (const duration of report.durations) {
    console.log(`Completion: job=${duration.job || "?"} key=${duration.key || "?"} jobMs=${duration.jobMs ?? "unavailable"} endToEndMs=${duration.endToEndMs ?? "unavailable"} queueWaitMs=${duration.ingressMs ?? "unavailable"} admissionMs=${duration.admissionMs ?? "unavailable"} legacyIngressBuildMs=${duration.legacyIngressBuildMs ?? "unavailable"}`);
  }
...
function assertIngressAdmissionSelfTests(logText, options) {
  const missingAdmission = evaluateLog(logText.replace(" admissionMs=24.000000", ""), options);
  if (missingAdmission.checks.find((check) => check.id === "admission-latency-within-limit")?.status !== "FAIL") {
    throw new Error("Self-test mutation: missing admissionMs did not fail.");
  }
  const slowAdmission = evaluateLog(logText.replace("admissionMs=24.000000", "admissionMs=1001.000000"), options);
  if (slowAdmission.checks.find((check) => check.id === "admission-latency-within-limit")?.status !== "FAIL") {
    throw new Error("Self-test mutation: slow admission did not fail.");
  }
}

function assertBoundedBuildSelfTests(logText, options) {
  const missingFinalBuild = evaluateLog(logText.replace("[08/14/2026 - 06:06:35PM] [PDV][QR_QUEUE] BUILD completed v3qr_5 key=Skyrim.esm|221587|220 cells=7 sourceCells=22 skipped=15 meta=0 materializeMs=10000.000000", ""), options);
  if (missingFinalBuild.checks.find((check) => check.id === "expected-build-sequence")?.status !== "FAIL") {
    throw new Error("Self-test mutation: missing bounded BUILD marker did not fail.");
  }
}
...
function main() {
  ...
  if (options.selfTest) {
    ...
    assertIngressAdmissionSelfTests(logText, options);
    assertBoundedBuildSelfTests(logText, options);
  }
