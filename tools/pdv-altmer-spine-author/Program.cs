...
const string substrateEdid = "PDV_Substrate_AltmerAncestor";
    ...
    AuthorReport report)
{
    ...
    ConfigureQuestShell(substrate, "Altmer Ancestor Spine");
    var substrateProperties = new ScriptProperty[]
    {
        StringProp("SubstrateName", "AltmerAncestor"),
        IntProp("RequiredOriginRace", 3),
        ObjectProp("PDV_GLO_OriginRace", originGlobal.FormKey),
        ObjectProp("PDV_GLO_DebugLevel", debugGlobal.FormKey),
        ObjectProp("Substrate_Always", RequireRecord<Spell>(index, "PDV_Bless_Altmer_Spine_Always").FormKey),
        ObjectProp("Substrate_Mid", RequireRecord<Spell>(index, "PDV_Bless_Altmer_Spine_Mid").FormKey),
        ObjectProp("Substrate_High", RequireRecord<Spell>(index, "PDV_Bless_Altmer_Spine_High").FormKey),
    };
    WireQuestScript(substrate, "PDV_SubstrateBase", substrateProperties);
    WireQuestScript(substrate, "PDV_Substrate_AltmerAncestor", substrateProperties);
    report.Actions.Add("Wired PDV_Substrate_AltmerAncestor base and concrete scripts.");

    var manager = RequireRecord<Quest>(index, managerEdid);
    WireQuestScript(manager, "PDV__ManagerQuest", new ScriptProperty[]
    {
        ObjectProp("PDV_AltmerAncestorSubstrate", substrate.FormKey),
    });
    report.Actions.Add("Wired PDV_AltmerAncestorSubstrate on PDV__ManagerQuest.");
    ...
    AuthorReport report)
{
    ...
    if (substrate is not null)
    {
        ...
        var concrete = CheckQuestScript(substrate, "PDV_Substrate_AltmerAncestor", report);
        if (concrete is not null)
        {
            CheckStringProperty(concrete, "SubstrateName", "AltmerAncestor", substrateEdid, report);
            CheckIntProperty(concrete, "RequiredOriginRace", 3, substrateEdid, report);
            ...
            CheckObjectProperty(concrete, "Substrate_Always", RequireRecord<Spell>(index, "PDV_Bless_Altmer_Spine_Always").FormKey, substrateEdid, report);
            CheckObjectProperty(concrete, "Substrate_Mid", RequireRecord<Spell>(index, "PDV_Bless_Altmer_Spine_Mid").FormKey, substrateEdid, report);
            CheckObjectProperty(concrete, "Substrate_High", RequireRecord<Spell>(index, "PDV_Bless_Altmer_Spine_High").FormKey, substrateEdid, report);
        }
    }
    ...
    if (manager is not null)
    {
        ...
        {
            CheckObjectProperty(script, "PDV_AltmerAncestorSubstrate", RequireRecord<Quest>(index, substrateEdid).FormKey, managerEdid, report);
        }
    }
...
static void CheckSpell(Dictionary<string, ISkyrimMajorRecordGetter> index, SpellSpec spec, AuthorReport report)
{
    ...
    {
        report.Errors.Add($"{spec.SpellEditorId} description does not match the Altmer substrate contract.");
    }
...
static void CheckMagicEffect(MagicEffect mgef, SpellSpec spellSpec, EffectSpec effectSpec, AuthorReport report)
{
    ...
    {
        report.Errors.Add($"{effectSpec.EffectEditorId} description does not match the Altmer substrate contract.");
    }
...
static void WriteMod(SkyrimMod mod, string espPath, AuthorReport report)
{
    ...
    var backupDir = Path.Combine(Path.GetDirectoryName(Path.GetFullPath(espPath))!, "Backups", "altmer-spine");
    ...
    var tempPath = $"{espPath}.altmer-spine.tmp";
...
[
    new(
        "PDV_Bless_Altmer_Spine_Always",
        "Altmer Spine - Ordered Heritage",
        "Auri-El's line holds steady. Maximum Magicka +10.",
        [
            new("PDV_MGEF_Altmer_Spine_Always_Magicka", "Altmer Ordered Heritage Magicka", "Magicka", 10.0f),
        ]),
    new(
        "PDV_Bless_Altmer_Spine_Mid",
        "Altmer Spine - Disciplined Heritage",
        "Aldmeri discipline sharpens the returnward path. Maximum Magicka +20.",
        [
            new("PDV_MGEF_Altmer_Spine_Mid_Magicka", "Altmer Disciplined Heritage Magicka", "Magicka", 20.0f),
        ]),
    new(
        "PDV_Bless_Altmer_Spine_High",
        "Altmer Spine - Exemplar Heritage",
        "The old claim burns clean and bright. Maximum Magicka +30.",
        [
            new("PDV_MGEF_Altmer_Spine_High_Magicka", "Altmer Exemplar Heritage Magicka", "Magicka", 30.0f),
        ]),
];
