...
[CmdletBinding()]
param(
    # ---- EDIT THIS ONCE if your mod folder lives elsewhere ----
...
# Repo root = parent of this script's folder (script lives in <repo>\tools\).
$RepoRoot = (Resolve-Path (Join-Path $PSScriptRoot "..")).Path

# Mapping: <repo-relative source>  ->  <DevotionRoot-relative destination>
$Map = @(
...
foreach ($entry in $Map) {
    ...
    if ($dstPath -like "*.psc") { $touchedPsc = $true }
    $copied++
}
