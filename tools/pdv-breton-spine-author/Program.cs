...
const string substrateEdid = "PDV_Substrate_BretonAncestor";
    ...
    AuthorReport report)
{
    ...
    ConfigureQuestShell(substrate, "Breton Ancestor Spine");
    var substrateProperties = new ScriptProperty[]
    {
        StringProp("SubstrateName", "BretonAncestor"),
        IntProp("RequiredOriginRace", 2),
        ObjectProp("PDV_GLO_OriginRace", originGlobal.FormKey),
        ObjectProp("PDV_GLO_DebugLevel", debugGlobal.FormKey),
        ObjectProp("Substrate_Always", RequireRecord<Spell>(index, "PDV_Bless_Breton_Spine_Always").FormKey),
        ObjectProp("Substrate_Mid", RequireRecord<Spell>(index, "PDV_Bless_Breton_Spine_Mid").FormKey),
        ObjectProp("Substrate_High", RequireRecord<Spell>(index, "PDV_Bless_Breton_Spine_High").FormKey),
    };
    WireQuestScript(substrate, "PDV_SubstrateBase", substrateProperties);
    WireQuestScript(substrate, "PDV_Substrate_BretonAncestor", substrateProperties);
    report.Actions.Add("Wired PDV_Substrate_BretonAncestor base and concrete scripts.");

    var manager = RequireRecord<Quest>(index, managerEdid);
    WireQuestScript(manager, "PDV__ManagerQuest", new ScriptProperty[]
    {
        ObjectProp("PDV_BretonAncestorSubstrate", substrate.FormKey),
    });
    report.Actions.Add("Wired PDV_BretonAncestorSubstrate on PDV__ManagerQuest.");
    ...
    AuthorReport report)
{
    ...
    if (substrate is not null)
    {
        ...
        var concrete = CheckQuestScript(substrate, "PDV_Substrate_BretonAncestor", report);
        if (concrete is not null)
        {
            CheckStringProperty(concrete, "SubstrateName", "BretonAncestor", substrateEdid, report);
            CheckIntProperty(concrete, "RequiredOriginRace", 2, substrateEdid, report);
            ...
            CheckObjectProperty(concrete, "Substrate_Always", RequireRecord<Spell>(index, "PDV_Bless_Breton_Spine_Always").FormKey, substrateEdid, report);
            CheckObjectProperty(concrete, "Substrate_Mid", RequireRecord<Spell>(index, "PDV_Bless_Breton_Spine_Mid").FormKey, substrateEdid, report);
            CheckObjectProperty(concrete, "Substrate_High", RequireRecord<Spell>(index, "PDV_Bless_Breton_Spine_High").FormKey, substrateEdid, report);
        }
    }
    ...
    if (manager is not null)
    {
        ...
        {
            CheckObjectProperty(script, "PDV_BretonAncestorSubstrate", RequireRecord<Quest>(index, substrateEdid).FormKey, managerEdid, report);
        }
    }
...
static void CheckSpell(Dictionary<string, ISkyrimMajorRecordGetter> index, SpellSpec spec, AuthorReport report)
{
    ...
    {
        report.Errors.Add($"{spec.SpellEditorId} description does not match the Breton substrate contract.");
    }
...
static void CheckMagicEffect(MagicEffect mgef, SpellSpec spellSpec, EffectSpec effectSpec, AuthorReport report)
{
    ...
    {
        report.Errors.Add($"{effectSpec.EffectEditorId} description does not match the Breton substrate contract.");
    }
...
static void WriteMod(SkyrimMod mod, string espPath, AuthorReport report)
{
    ...
    var backupDir = Path.Combine(Path.GetDirectoryName(Path.GetFullPath(espPath))!, "Backups", "breton-spine");
    ...
    var tempPath = $"{espPath}.breton-spine.tmp";
...
[
    new(
        "PDV_Bless_Breton_Spine_Always",
        "Breton Spine - Inherited Ward",
        "The old mixed blood remembers how to resist the impossible. Magic resistance +3%.",
        [
            new("PDV_MGEF_Breton_Spine_Always_ResistMagic", "Breton Inherited Ward", "ResistMagic", 3.0f),
        ]),
    new(
        "PDV_Bless_Breton_Spine_Mid",
        "Breton Spine - Warded Blood",
        "The old mixed inheritance stands between you and hostile magic. Magic resistance +7%.",
        [
            new("PDV_MGEF_Breton_Spine_Mid_ResistMagic", "Breton Warded Blood", "ResistMagic", 7.0f),
        ]),
    new(
        "PDV_Bless_Breton_Spine_High",
        "Breton Spine - Awakened Inheritance",
        "The old mixed inheritance answers cleanly. Magic resistance +12%.",
        [
            new("PDV_MGEF_Breton_Spine_High_ResistMagic", "Breton Awakened Inheritance", "ResistMagic", 12.0f),
        ]),
];
