...
const string defaultManifest = @"references\authoring\PDV_Phase20BosmerNonHunterImplementationCosting.manifest.json";
...
try
{
    ...
    {
        throw new FileNotFoundException("Bosmer implementation manifest not found.", manifestPath);
    }
    ...
    if (checkPlacements)
    {
        ...
        {
            throw new InvalidOperationException("Bosmer placement check failed.");
        }
    ...
    else
    {
        ...
        WriteModIfNeeded(mod, espPath, dryRun, report, "phase20-bosmer");
        report.Status = "PASS";
    }
}
catch (Exception ex)
...
static RedguardManifest LoadManifest(string manifestPath)
{
    ...
    if (parsed is null)
    {
        throw new InvalidOperationException("Bosmer implementation manifest could not be parsed.");
    }

    if (parsed.triggerSurfaces is null || parsed.triggerSurfaces.Count == 0)
    {
        throw new InvalidOperationException("Bosmer implementation manifest has no trigger surfaces.");
    }
...
static TriggerDefinition[] BuildTriggerDefinitions(RedguardManifest manifest)
{
    ...
    foreach (var trigger in manifest.triggerSurfaces ?? [])
    {
        ...
        {
            throw new InvalidOperationException($"Bosmer trigger surface {trigger.id ?? "(unknown)"} must be an ACTI record.");
        }

        if (string.IsNullOrWhiteSpace(trigger.editorId)
            || trigger.routeId <= 0
            || trigger.requiredOriginRace != 4
        ...
        {
            throw new InvalidOperationException($"Bosmer trigger surface metadata is incomplete for {trigger.id ?? "(unknown)"}.");
        }
