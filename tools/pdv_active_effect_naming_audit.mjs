...
async function main() {
  ...
  const findings = blocks(text(effectDetails), "MagicEffect").map((block) => { const actorValue = field(block, "Archetype.ActorValue"); const expected = labels.get(actorValue); const name = field(block, "Name"); const parent = parents.get(formId(block).toLowerCase()); const valid = expected ? name === expected : actorValue === "None" && name !== "" && name !== parent; return { status: valid ? "PASS" : "FAIL", editor: field(block, "EditorID"), name, parent, actorValue, expected: expected ?? "a distinct scripted-effect name" }; });
