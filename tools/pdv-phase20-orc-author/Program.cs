...
const string defaultManifest = @"references\authoring\PDV_Phase20OrcImplementationCosting.manifest.json";
const string orcLifeModeTrack = "PDV_StateTrack_OrcLifeMode";
...
try
{
    ...
    {
        throw new FileNotFoundException("Orc implementation manifest not found.", manifestPath);
    }

    var manifest = LoadManifest(manifestPath);
    var lifeModeLabels = GetStateLabels(manifest, orcLifeModeTrack);
    ...
    if (checkPlacements)
    {
        ...
        {
            throw new InvalidOperationException("Orc placement check failed.");
        }
    ...
    else
    {
        ...
        var lifeModeGlobal = RequireRecord<Global>(index, "PDV_GLO_OrcLifeMode");
        var eventBus = RequireRecord<Quest>(index, "PDV_EventBus");
        var playerRef = new FormKey(ModKey.FromNameAndExtension("Skyrim.esm"), 0x14);

        var lifeModeTrack = createMissing
            ? EnsureQuest(mod, index, allocator, orcLifeModeTrack, report)
            : RequireRecord<Quest>(index, orcLifeModeTrack);
        ConfigureQuestShell(lifeModeTrack, orcLifeModeTrack);
        WireQuestScript(lifeModeTrack, "PDV_StateTrack", new ScriptProperty[]
        {
            StringProp("TrackName", "OrcLifeMode"),
            ObjectProp("StateGlobal", lifeModeGlobal.FormKey),
            ObjectProp("PDV_GLO_DebugLevel", debugGlobal.FormKey),
            StringListProp("StateLabels", lifeModeLabels),
        });
        report.Actions.Add("Ensured PDV_StateTrack_OrcLifeMode is wired to PDV_StateTrack.");
        ...
        {
            ObjectProp("PDV_OrcLifeModeTrack", lifeModeTrack.FormKey),
        });
        report.Actions.Add("Wired Orc life-mode property on PDV__ManagerQuest.");

        WriteModIfNeeded(mod, espPath, dryRun, report, "phase20-orc");
        report.Status = "PASS";
    }
}
catch (Exception ex)
...
static OrcManifest LoadManifest(string manifestPath)
{
    var parsed = JsonSerializer.Deserialize<OrcManifest>(File.ReadAllText(manifestPath));
    if (parsed is null)
    {
        throw new InvalidOperationException("Orc implementation manifest could not be parsed.");
    }

    if (parsed.stateSurfaces is null || parsed.stateSurfaces.Count == 0)
    {
        throw new InvalidOperationException("Orc implementation manifest has no state surfaces.");
    }

    return parsed;
}

static string[] GetStateLabels(OrcManifest manifest, string editorId)
{
    ...
    {
        throw new InvalidOperationException($"Orc implementation manifest is missing {editorId} enum.");
    }
    ...
    if (ordered.Length != 3 || ordered[0] != "City" || ordered[1] != "Stronghold" || ordered[2] != "LegionExile")
    {
        throw new InvalidOperationException($"{editorId} enum does not match the locked Orc life-mode contract.");
    }

    return ordered;
}

static TriggerDefinition[] BuildTriggerDefinitions(OrcManifest manifest)
{
    ...
    foreach (var trigger in manifest.triggerSurfaces ?? [])
    {
        ...
        {
            throw new InvalidOperationException($"Orc trigger surface {trigger.id ?? "(unknown)"} must be an ACTI record.");
        }

        if (string.IsNullOrWhiteSpace(trigger.editorId)
            || trigger.routeId <= 0
            || trigger.requiredOriginRace != 8
        ...
        {
            throw new InvalidOperationException($"Orc trigger surface metadata is incomplete for {trigger.id ?? "(unknown)"}.");
        }
