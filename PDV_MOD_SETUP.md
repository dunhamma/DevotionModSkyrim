...
**2026-07-13 AEST - Breton architecture audit and Hidden Art layered-pact repair:** Runtime co-test passed the Hidden Art Champion capstone, Prince boon, price waiver, and current-pool preservation, but Survey showed only the Prince. Root cause was `GetSurveyDevotionText()` applying the global Prince-wins return before the Breton renderer. Breton now dispatches to `GetBretonSurveyText()` first, which preserves tradition/practice/exposure and appends the active integrated Prince. The same audit removed Notorious practice/reward suppression, implemented the locked 1.25 Notorious gain for the four integrated Princes, waived/cleared duplicate generic stigma, reconciled real startup and debug tradition state immediately, and preserves a layered panel note. `tools/pdv_breton_architecture_audit.mjs` is the fail-closed static gate. Repository and live manager/base source hashes match after backup under `generated/live-devotion-backups/breton-architecture-20260713-182438`; both scripts compile 0/0. Manual proof remaining: Survey composition only.
