...
| `references/authoring/PDV_Phase21_DoD_AuthorHandoff.md` | Diaries of Dibella compatibility handoff: Wintersun removal set, load-order placement, no-ESP rationale, generated-output rebuild requirement, extensibility boundary, and smoke checklist |
| `references/authoring/PDV_Phase21_DoD_AuthoriaReuseAudit.md` | Diaries of Dibella / Authoria reuse audit for shared quest, custom-race, shrine, and social-location surfaces before any DoD extensibility adapter ships |
| `references/authoring/PDV_Phase21_DoD_PackageInstall.md` | Diaries of Dibella MO2 install guide for `dist/PDV_DoD_BordelloPatch_v0_20260615.zip`, Wintersun removal, Devotion placement, and Synthesis/ParallaxGen/TexGen/DynDOLOD reruns |
| `references/authoring/PDV_Phase21_DoD_CompatibilityPackage.manifest.json` | Diaries of Dibella package manifest for local evidence, packaged runtime files, disabled Wintersun-family plugins, shrine readback, generated-output rerun requirement, and future adapter scope |
...
**2026-06-15 AEST - Diaries of Dibella Phase 21 package evidence:** DoD is now at local `patch-packaging` evidence for the Wintersun replacement slice. The test profile is `D:\Wabbajack\modlists\DoD`, profile `Diaries of Dibella - Lord's Vision`, with `D:\Wabbajack\modlists\DoD\mods\Devotion - PlayerDevotion Local Test` junctioned to the Anvil Devotion mod. Profile backups are under `D:\Wabbajack\modlists\DoD\profiles\Diaries of Dibella - Lord's Vision\pdv-dod-backups`, stamp `20260615-130025`. Live profile inspection found 20 Wintersun-family plugin lines; all 20 are disabled locally, and `Devotion.esp` is active at the former Wintersun core slot. houseCARL readback confirmed all 14 shrine blessing spells now win from `Devotion.esp`, with Wintersun inactive and the Dragonborn Good Daedra altar spells preserving `PDV_MGEF_DunmerShrineCure` / `PDV_DunmerShrinePrayerEffect`. The shareable package is `dist\PDV_DoD_BordelloPatch_v0_20260615.zip`; it is MO2-ready and includes `Devotion.esp`, compiled scripts, SEQ, SKSE runtime data, and docs, while excluding source PSC files, backups, candidate overlays, and generated outputs. Local generated-output inspection found stale Wintersun masters in `DynDOLOD.esp`, `Lord's Vision - Synthesis Gameplay.esp`, and `PG_1.esp`, so reusable installs must rerun Synthesis, ParallaxGen, TexGen if required, and DynDOLOD after Wintersun removal. Authoria/ARR reuse comparison found shared Half-Khajiit, Heart of Dibella QE, Caught Red Handed QE, Talos' Tease, The Only Cure QE, and Whispering Door QE surfaces, but ARR's Reqtificated plugins are Requiem/list-balancing overrides and must not be copied into DoD. No standalone `PDV_DoD_Compatibility.esp` was emitted for this proven slice because no extra DoD override is needed; the name remains reserved for future approved Wintersun asset reuse, Sybil blessing, or curated social-route adapters. Ohmes-Raht / Half-Khajiit now ships as a data-only origin map in `PDV_RaceMap.json`, mapping `0x03322B|HalfKhajiit.esp` and `0x05693A|HalfKhajiit.esp` to Khajiit origin index `6` without adding `HalfKhajiit.esp` as a Devotion master. Extensibility readback found `akdDibellanBathsLocation` already has `LocTypeTemple`; `akdDibellaMarker` points at vanilla `AltarDibellaSpell` and is covered; `akdSybilMarker` points at custom `akdAltarSybilSpell` and is candidate-only; `XXTUSolitudeBordello`, `XXTURiftenBrothel`, and `JOJ_TalosTeaseLocation` are social/venue context, not temple patches. Package docs: `references\authoring\PDV_Phase21_DoD_CompatibilityPackage.manifest.json`, `references\authoring\PDV_Phase21_DoD_AuthoriaReuseAudit.md`, `references\authoring\PDV_Phase21_DoD_PackageInstall.md`, and `references\authoring\PDV_Phase21_DoD_AuthorHandoff.md`. Runtime/manual DoD smoke and current public/author evidence refresh remain open.
