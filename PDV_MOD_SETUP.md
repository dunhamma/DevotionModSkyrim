...
| `references/authoring/PDV_MedallionRoster.manifest.json` | Phase 20 intent-plus-readback medallion roster: all ten race native rosters stay visible, while only live `PDV_FLST_AllDeities` members are selectable |
...
| `tools/pdv-shrine-blessing-author` | Narrow direct-framework helper that discovers clickable shrine blessing activators and creates/checks cure-only main-ESP shrine blessing `SPEL` overrides from `references/authoring/PDV_ShrineBlessingNeutralization.manifest.json`; it does not replace activator scripts |
...
`tools\pdv-phase20-reward-author` is a narrow direct-framework helper for the Phase 20 all-race first-tier reward wave. It reads `references\authoring\PDV_Phase20_RewardRecordContracts.json`, creates/checks the ten race reward `SPEL` records and their `MGEF` value-modifier effects, and wires matching reward spell properties on `PDV__ManagerQuest`. Run `--create-missing --dry-run` first. After Skyrim/Creation Kit and load-order MCP readers release `PlayerDevotion_Framework.esp`, run `--create-missing`, `--wire-manager`, and `--check`; then compile touched scripts and rerun the strict verifier ladder. The helper does not replace runtime/manual proof for Active Effects display, save/load behavior, Survey clarity, or balance feel.
...
Phase 9 runtime proof is closed as of 2026-05-24. The proof pass covered all five Bosmer proof activator routes (`41-45`), Living Story/Exchange/Bandit Road/Old Contract offers, popup accept paths, confirmation-rite state switching, Old Contract `PactBound` and `GreenPactCompliance` separation, forced reckoning `Renounce`, forced reckoning `Recommit`, and save/load persistence. The proof pass also fixed `PDV_StateTrack` evidence retention so three-day offers retain/count `LatestDay`, `PreviousDay`, and `ThirdDay`.
...
- For Phase 12 contextual-favor pilot closeout, run `dotnet run --project .\tools\pdv-phase12-author -- --dry-run --create-missing`, then the live `--create-missing` pass if the dry-run is correct, then `node .\tools\pdv_verify.mjs --strict-phase12`, then your full upstream strict ladder before declaring runtime proof ready. A fresh ESP write should leave `SEQ freshness` as the expected remaining warning until `PlayerDevotion_Framework.seq` is refreshed.
...
**2026-05-23 - Phase 9 framework record and placement closeout:** The Bosmer path framework packet is now authored directly into `PlayerDevotion_Framework.esp` with timestamped backups under `D:\Wabbajack\modlists\Anvil\mods\Devotion\Backups\phase9\`. Live record readback confirms `PDV_Deity_Zen`, `PDV_Deity_BaanDar`, Y'ffre/Zen/BaanDar path eligibility, all six Bosmer message boxes, `PDV__ManagerQuest` Bosmer properties, `PDV_FLST_AllDeities` membership, and the five ACTI proof-surface base records for route IDs `41-45`. CK-authored proof placements now also read back correctly: `PDV_REFR_BosmerLivingStorySignal`, `PDV_REFR_BosmerExchangeSignal`, `PDV_REFR_BosmerBanditRoadSignal`, `PDV_REFR_BosmerPactPositiveSignal`, and `PDV_REFR_StateTransitionConfirmRite` each point at the intended ACTI base. The full strict gate is clean after SEQ freshness refresh: `node .\tools\pdv_verify.mjs --strict-phase9 --strict-phase8 --strict-phase7 --strict-preflight --strict-skeleton --strict-pattern-proving --json` => `PASS=808, WARN=0, FAIL=0, INFO=28`. This entry captured the framework/placement checkpoint; the counted runtime proof was completed on 2026-05-24.
