...
**2026-07-13 AEST - Breton architecture audit and Hidden Art layered-pact repair:** Runtime co-test passed Book of Days, the Hidden Art Champion route, Prince boon, price waiver, and current-pool preservation, but Survey showed only the Prince and Active Effects retained both Hidden Art T2 and T3. The Survey renderer source was correct but had been synced after the manager PEX compile; preserved source timestamps let freshness checks miss that ordering error. `pdv_compile.mjs` now refuses tracked/deployed source drift before compiling. Breton same-family Champion records are cumulative absolute totals and now replace T2, while a distinct non-resonant patron boon remains beside T2. The broader repair keeps the Breton Survey base, removes Notorious practice/reward suppression, implements the locked 1.25 integrated-Prince gain, waives/clears duplicate generic stigma, reconciles startup/debug state, and preserves a layered panel note. Quest-matrix optional-plugin form resolution now checks plugin presence before `GetFormFromFile`, preventing repeated Papyrus errors for absent integrations; the declaration gate and default verifier enforce that ordering. Clean deployment completed with timestamped source/PEX backups under `Backups\breton-hidden-art-20260713-191506`, `Backups\breton-hidden-art-presentation-20260713-193543`, and `Backups\daedric-script-attachment-cleanup-20260713-200507`; manager, PlayerEvents, MCM, and Survey compiled 0/0 with matching tracked/live source hashes and fresh PEX timestamps. Runtime retest passed Active Effects, journal/Book of Days, and layered Survey; log proved T2 removal plus Champion addition and no price spell. The later red `+35` toast matched neither Mora's live Champion boon (`+20 Alteration`) nor his Champion price (`-30 Stamina`): it exposed stale hardcoded presentation data. The manager now derives an audit-checked all-Prince presentation table from the 96-row record contract, marks boon payloads green, suppresses waived price copy, and replays Champion offers through the concrete Prince script. All 16 path quests had a redundant direct `PDV_DaedricPathBase` VMAD attachment removed; each now reads back with exactly one concrete script, and `pdv_verify.mjs` fails closed on that invariant. Survey uses one sentence: `Your pact with Hermaeus Mora has opened Hidden Art - Champion.` Final deployed gates: Breton audit 32/32, Prisma 95/95, toast fallback 26/26, Book of Days 126/126, Phase 2 reward readback 1482/1482, and verifier `PASS=3581 WARN=2 FAIL=0`. Manual proof remaining after deployment: controlled offer appears, acceptance emits the green `+20 Alteration` boon toast, and no price toast appears.
