...
| `tools/pdv_copy_census.mjs` | Read-only player-copy census: batches direct houseCARL reads of live `Devotion.esp`, inventories tracked Papyrus/MCM and Prisma strings, excludes developer `Debug*` functions, links race/Daedric manifest references without promoting them, and writes only ignored local census, formal-offer, dynamic-narrative viability, Nord/Kyne packet, full Penpot-importable journey map, stable copy-to-flow assignments, Excel-readable UX surface catalogue, editable Penpot surface library, and popup writing-limit CSV/Penpot frames. `--check` fails if any live row lacks a reviewed flow family or the surface authority is invalid. |
| `tools/pdv_implementation_audit_export.mjs` | Read-only atlas/workbook reconciliation gate: validates stable atlas node IDs and the actionable implementation queue against the 5,166-row census/exchange, derives exact workbook edit locations, and emits ignored JSON/CSV projections. It consumes direct houseCARL findings already recorded in the atlas; it is not a plugin reader or writer. |
...
node --test .\tools\pdv_copy_census.test.mjs .\tools\pdv_copy_roundtrip.test.mjs .\tools\pdv_ux_surface_catalogue.test.mjs
