...
| `references/authoring/PDV_Phase20_NoInGameProof_Workplan.md` | Remaining Phase 20 no-in-game-proof queue for gate-ledger hardening, immersive hook contracts, static verifier work, stack audits, CAT-6 prep, recognition prep, and Daedric blocker closeout |
...
| `references/authoring/PDV_Phase20_NextReadbackHook.md` | Next-session hook for the all-race exact-source readback pass; produces an approval dossier before any live source fill |
...
| `references/authoring/PDV_Phase20_P2ImmersiveReceivers.manifest.json` | Source-scaffolded Breton, Dunmer, Imperial, and Nord receiver manifest for player-alias PO3 book, spell, harvest, weather, and quest-stage hooks, with FLST shell, alias-property, source-fill tooling, and receiver anti-farm readback metadata |
| `references/authoring/PDV_Phase20_P2SourceCuration_Runbook.md` | P2 source curation handoff for deciding exact Breton, Dunmer, Imperial, and Nord source records before approved source-fill writes |
...
| `tools/pdv-phase20-p2-receiver-author` | Narrow direct-framework helper that creates/checks the empty `PDV_FLST_P2_*` receiver FormList shells for Breton, Dunmer, Imperial, and Nord immersive hooks, wires/checks those FormLists on the existing `PDV_PlayerEvents` alias script, and fills/checks explicitly approved exact source entries from the receiver manifest |
