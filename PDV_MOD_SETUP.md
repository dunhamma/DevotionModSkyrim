...
| `tools/pdv-phase20-race-author` | Race-agnostic direct-framework reward/spec author that creates/wires per-race deity, reward, neglect, support-spell, state-track/global mirror, and message-record packets from `PDV_{Race}RewardRecords.spec.json`; regen AVs are authored as PeakValueModifier effects |
