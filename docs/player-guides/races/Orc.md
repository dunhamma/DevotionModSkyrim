...
- **Read the rare worthy texts and visit Malacath's holds.** Approved Orc texts and shrine moments register a little. Standing at each of the four strongholds (Dushnikh Yal, Mor Khazgur, Narzulbur, Largashbur) is recognized once each, with a milestone for reaching all four. Living rough, resting under the open sky like the exile, also earns a small note. `[WIRED (holds + open sky) / PARTIAL (texts): the four-holds visit fires organically via HandleStoryChangeLocation -> HandleOrcLocationChange -> GetOrcStrongholdHoldId -> SIGNAL_FOUR_HOLDS_VISIT (+1.0, once per hold) + SIGNAL_ANCESTOR_SPINE (+1.0); location presence also feeds Stronghold mode evidence. Rest-under-open-sky (313) +0.25 fires via the CSV OnSleepStop hook. "Worthy texts" = SIGNAL_BROAD_CONDUCT (+1.0) via RouteOrcMalacathConduct, organic only from PDV_FLST_P2_OrcMalacathSources book reads (PO3 source hooks) and DA06 stage; the dev-only route is separate.]`
...
- **Orc spine by mode.** The Orc reward spine (DamageResist / Health armor-points) is delivered through the tier reward spells per mode (Stronghold Smithing/Heavy Armor, City Restoration/Speech/Block, Legion One-Handed/Block), all three Champions converging on Armor +50. Per the DamageResist memo, Orc uses armor POINTS 15/30/50; this is the reward layer, not the gain layer.
