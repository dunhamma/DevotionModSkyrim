...
| `tools/pdv-phase20-p2-receiver-author` | Narrow Mutagen-backed Phase 20 P2 receiver helper | Creating/checking the empty `PDV_FLST_P2_*` receiver FormList shells for Breton, Dunmer, Imperial, and Nord immersive PO3/player-alias hooks, wiring/checking those FormLists on the existing `PDV_PlayerEvents` player-alias script, and filling/checking explicitly approved exact source entries when the source manifest is curated |
...
| `references/authoring/PDV_Phase20_SourceFillApprovalLedger.json` | Phase 20 source-fill approval ledger | Recording approved P2 book-read source fills, blocked quest-stage/non-P2 receiver decisions, duplicate guards, and verification commands before live FormList fill |
| `references/authoring/PDV_Phase20_P2ImmersiveReceivers.manifest.json` | Phase 20 P2 immersive receiver manifest | Tracking the source-scaffolded Breton, Dunmer, Imperial, and Nord PO3/player-alias receiver FormLists, alias-property wiring, source-fill tooling readiness, receiver-side anti-farm policy, event families, route targets, asset status, and remaining curated-source/runtime proof boundary |
| `references/authoring/PDV_Phase20_P2SourceCuration_Runbook.md` | Phase 20 P2 source curation handoff | Separating candidate Breton/Dunmer/Imperial/Nord book, spell, and quest-stage source directions from approved `sourceFillEntries`; use before adding any live P2 FormList entries |
...
- **Phase 20 pre-beta gate implementation (2026-05-31 AEST):** `references/authoring/PDV_PreBetaRaceGateLedger.md` now records all ten race gate packets with verdict, normal-session route, accepted/rejected hooks, anti-farm rule, Survey/status result, final placement result, reward floor/ceiling, stack snapshot, expected/edge builds, and blocking follow-up. The Altmer costing manifest is reconciled to the current content-lock rows (`MarriageBeat`, `DivineBody_DawnObservance`, `ThalmorOrthodox_ProjectDefended`) while preserving legacy wired record EditorIDs. `PDV__ManagerQuest.psc` now has fiction-facing Survey/status branches for all ten races, including P2 Breton/Dunmer/Imperial audit readouts and less numeric Argonian/Redguard status text. `tools/pdv_verify.mjs --strict-phase20-race-costing` now checks the all-race Survey source scaffold, and `PDV_Phase20_PreBetaManualChecks_Runbook.md` owns the manual handoff. Daedric docs now treat all sixteen Skyrim-present Prince rows as drafted, with promotion still gated by stigma, Hircine/Molag Bal curse-access, and Prince-order decisions.
...
- **Phase 20 no-in-game-proof workplan (2026-05-31 AEST; recognition scope revised 2026-06-01):** `references/authoring/PDV_Phase20_NoInGameProof_Workplan.md` now defines the rest of Phase 20 work that can continue before additional Skyrim runtime proof: gate-ledger hardening, normal-play hook contracts, static verifier expansion, final-world placement contracts, stack/ceiling audits, Survey/status copy prep, CAT-6 target-record proof, recognition packet prep, and Daedric blocker closeout. `references/authoring/PDV_Phase20_NoInGameProof_Gates.json` is the structured gate packet now checked by `tools/pdv_verify.mjs --strict-phase20-race-costing`: all ten races stay `Fail - runtime/manual proof deferred`, P0/P1 lanes have final-placement contracts, P2 lanes remain audit-only, CAT-6 has one pilot-provisional Khajiit Tier 1 `SPEL` plus two night-gated `MGEF` records, recognition remains planned V2 prep only, and Daedric runtime promotion remains blocked on stigma, curse-access template, and Prince order. `references/authoring/PDV_Phase20_ManualEvidenceLedger.json` is the matching pending evidence intake, also checked by the strict gate so future manual proof has slots without silently becoming a pass claim. The lane explicitly forbids marking race `Pass`, claiming final placement, or broad-promoting rewards/strings without manual/runtime evidence.
- **Phase 20 P2 source readback and safe book fill (2026-06-03 AEST):** Phase 20 source curation now has a readback-backed quest-stage inventory, all-race dossier, exclusion audit, source-fill approval ledger, and beta-readiness remainder hook. `PDV_Phase20_P2ImmersiveReceivers.manifest.json` now contains 12 approved P2 book-read source entries across 6 groups for Breton Hidden Art, Dunmer Azura, Dunmer Boethiah, Imperial Talos pressure, Nord Old Ways, and Nord Hircine/Arkay edge. The live fill wrote only those approved book entries to `PlayerDevotion_Framework.esp` with backup `D:\Wabbajack\modlists\Anvil\mods\Devotion\Backups\phase20-p2-receivers\PlayerDevotion_Framework.esp.20260603-160221.bak`; quest-stage and non-P2 fills remain blocked until exact-stage receiver support and receiver/FormList safety exist. `--check-source-fill`, `--check-formlists`, and `--check-alias-properties` pass, SEQ was refreshed with backup `D:\Wabbajack\modlists\Anvil\mods\Devotion\Seq\PlayerDevotion_Framework.seq.20260603-060425.bak`, `node .\tools\pdv_content_verify.mjs` is clean at `FAIL=0, WARN=0, PASS=1079, INFO=4`, and `node .\tools\pdv_verify.mjs --strict-phase20-altmer --strict-phase20-race-costing --json` is clean at `PASS=2630, WARN=1, INFO=29` with only the existing unnamed CK-authored INFO warning. This is source-fill/readback proof, not beta readiness by itself.
