...
| `tools/pdv-phase20-cat6-author` | Narrow Mutagen-backed Phase 20 CAT-6 helper | Creating/checking the pilot-provisional `PDV_Bless_Khajiit_Lunar_T1` SPEL plus night-gated stamina/disease MGEFs from the exact race content manifest row; no manager grant wiring |
...
| `references/authoring/PDV_RaceEffectReviewLedger.md` | Race effect-review gate ledger | Locking race-by-race effect families, magnitudes, conditions, stacking, grant/removal, Survey explanation, anti-farm posture, and curse/Daedric interactions before broad reward authoring |
...
- **Phase 20 no-in-game-proof workplan (2026-05-31 AEST; recognition scope revised 2026-06-01; Daedric status refreshed 2026-06-04):** `references/authoring/PDV_Phase20_NoInGameProof_Workplan.md` now defines the rest of Phase 20 work that can continue before additional Skyrim runtime proof: gate-ledger hardening, normal-play hook contracts, static verifier expansion, final-world placement contracts, stack/ceiling audits, Survey/status copy prep, CAT-6 target-record proof, recognition packet prep, and Daedric proof-path closeout. `references/authoring/PDV_Phase20_NoInGameProof_Gates.json` is the structured gate packet now checked by `tools/pdv_verify.mjs --strict-phase20-race-costing`: all ten races stay `Fail - runtime/manual proof deferred`, P0/P1 lanes have final-placement contracts, P2 lanes remain audit-only, CAT-6 has one pilot-provisional Khajiit Tier 1 `SPEL` plus two night-gated `MGEF` records, recognition remains planned V2 prep only, and Daedric runtime promotion remains blocked on the locked D-18/CAT-6 proof path, including target-record readback, runtime/display proof, and stack/Survey legibility. `references/authoring/PDV_Phase20_ManualEvidenceLedger.json` is the matching pending evidence intake, also checked by the strict gate so future manual proof has slots without silently becoming a pass claim. The lane explicitly forbids marking race `Pass`, claiming final placement, or broad-promoting rewards/strings without manual/runtime evidence.
...
- **CAT-6 Khajiit Tier 1 pilot (2026-05-31 AEST):** `tools/pdv-phase20-cat6-author --create-missing` created/finalized `PDV_Bless_Khajiit_Lunar_T1`, `PDV_MGEF_Bless_Khajiit_Lunar_T1_StaminaRegen`, and `PDV_MGEF_Bless_Khajiit_Lunar_T1_DiseaseResist` in the framework ESP from the exact race content manifest row. Both effects are night-gated from 7PM through 7AM; the source copy now says "At night" to match. The framework backup is `D:\Wabbajack\modlists\Anvil\mods\Devotion\Backups\cat6-khajiit-lunar-t1\PlayerDevotion_Framework.esp.20260531-184518.bak`. The pilot is grant-unwired and provisional: it proves source-to-record readback, not Active Effects runtime display, reward distribution, or broad race reward policy. `references/authoring/PDV_RaceEffectReviewLedger.md` now owns the holistic race-by-race effect review before broader reward authoring.
