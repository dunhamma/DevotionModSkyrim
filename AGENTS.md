...
| `tools/pdv_quest_reaction_performance_audit.mjs` | Read-only V3 Quest Reaction ownership audit | Verifying the Runtime-owned bounded FIFO, one scheduler, V3 persistence namespace, Manager callback boundary, ordinary-scope closure before QR submission, lightweight admission, persisted bounded catalog materialization, direct EventBus/MCM routing, and Worker retirement |
