...
| `references/authoring/PDV_RaceArchitectureAtlas.json` | Race architecture atlas authority (shared Claude/Codex) | Reading or updating the high-level per-race architecture map: all 10 races across Spine / Substrate / Curse lanes, the shared Daedric track, and the cross-race matrix, each node carrying `status` (`built`/`partial`/`designed`/`absent`/`na`) plus file evidence. Edit node statuses HERE -- this JSON is the authority and the HTML is generated. Current statuses are static Papyrus-source verification: "MESG exists" means a declared property with a live show path, NOT an ESP readback, so a houseCARL-verified node is an upgrade worth recording. |
| `tools/pdv_atlas_render.mjs` | Race atlas renderer and data gate | Rendering the atlas JSON to standalone HTML (`node tools/pdv_atlas_render.mjs`, default output `generated/PDV_RaceArchitectureAtlas.html`, gitignored). `--check` validates without rendering and the verdict is the exit code: a bad `status` value or a matrix row of the wrong width exits 1. Never commit or hand-edit the HTML -- it is a generated report, and editing it would give one data item two backends. |
