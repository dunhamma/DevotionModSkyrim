...
- **[2026-08-21] - UX surface exploration is a regenerable design layer, not implementation authority:** The copy workbench now generates a 20-template Skyrim/PDV surface catalogue and an editable Penpot SVG library from `PDV_UXSurfaceCatalogue.json`. Templates cover established Skyrim surfaces, PDV's existing stack, optional framework-backed surfaces, and bespoke UI, with stable IDs, dependency/cost constraints, cautions, fallbacks, and implementation routes. Owner decisions remain editable in the Excel-readable catalogue while exact prose remains in the prose exchange. Choosing or modifying a template does not install a dependency, edit the ESP, or authorize wire-in; those decisions require a later reviewed implementation tranche.
