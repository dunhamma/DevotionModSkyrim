...
| `tools/pdv_quest_reaction_build.mjs` | Deterministic V3 Quest Reaction catalog compiler/checker | Generating or exact-byte-checking the two fully-qualified v2 catalogs and SHA-256 receipt from the compatibility manifest plus canonical CSV inputs; backend proof only until Runtime cutover |
