...
| `references/authoring/PDV_AllRaceDaedricBetaReadinessLedger.md` | Combined beta-feel blocker ledger | Tracking the current all-ten-race plus all-sixteen-Prince beta-feel gap, including P2 runtime proof, manual evidence, Daedric 20C proof, and CAT-6 promotion/readback blockers |
...
| `references/authoring/PDV_SessionHandoff_Phase2_AllRaces.md` | Phase 2 all-race propagation handoff (current) | Session detail for Phase A design + B1 deity scripts (22/22 compile 0/0) + B2 Argonian pilot landed; what's left in dependency order; operational gotchas; full reference data for resumption |
...
[~] Phase 2 all-race propagation (2026-06-07) - design + B1 scripts + Argonian B2 pilot complete; B2 remainder + capstone MGEFs + Bosmer Green Pact tag layer + ESP authoring + runtime proof pending
      ...
      - B2 (manager wiring, serial on `PDV__ManagerQuest.psc`): Argonian PILOT LANDED & compiles
        0/0 - declared `PDV_Hist`/`PDV_Sithis` deity props + 8 reward + neglect Spell props;
        double-routed 4 Argonian handlers to also pulse Hist (+ Sithis when `IsVoidFullyActive`);
        added `SyncArgonianRewards`/`SyncArgonianNeglectSpell` modelled on `SyncKhajiitEmphasisRewards`;
        no-offer special-case integrated. Remaining: Imperial/Altmer/Dunmer/Orc/Redguard/Nord/Bosmer/
        Breton wiring + Khajiit capstone back-fill, per-race one-at-a-time
      - Important B2 gap surfaced: per-race handlers (Imperial/Altmer/Orc/Redguard/Nord/Dunmer)
        are currently TELEMETRY STUBS - they increment StorageUtil counters but do NOT call
        `AwardCuratedSignal`. Bosmer/Yffre + Khajiit are the working templates; the Argonian pilot
        is the substrate variant. Imperial additionally needs a civic-act-type discriminator on
        `RouteImperialCivicService` before per-Divine scoring works (same shape likely for Nord
        Nine Divines lane)
      - Authoring order (owners precede reusers): Imperial+Argonian first -> Orc (before Altmer
        because Altmer reuses Trinimac) -> Dunmer/Altmer/Redguard -> Nord/Bosmer/Breton (reusers)
      - Open: `pdv_verify` expected-data may be stale re: the 22 new deities (per the prior
        Khajiit-pilot note) - resolve at Phase E; rule 4 requires explicit user OK before editing
        `pdv_verify.mjs` / `pdv_compile.mjs` / `pdv_author.mjs`
...
- **Phase 20 pre-beta gate implementation (2026-05-31 AEST; beta-feel ledger refreshed 2026-06-04):** `references/authoring/PDV_PreBetaRaceGateLedger.md` now records all ten race gate packets with verdict, normal-session route, accepted/rejected hooks, anti-farm rule, Survey/status result, final placement result, reward floor/ceiling, stack snapshot, expected/edge builds, and blocking follow-up. The Altmer costing manifest is reconciled to the current content-lock rows (`MarriageBeat`, `DivineBody_DawnObservance`, `ThalmorOrthodox_ProjectDefended`) while preserving legacy wired record EditorIDs. `PDV__ManagerQuest.psc` now has fiction-facing Survey/status branches for all ten races, including P2 Breton/Dunmer/Imperial audit readouts and less numeric Argonian/Redguard status text. `tools/pdv_verify.mjs --strict-phase20-race-costing` now checks the all-race Survey source scaffold, and `PDV_Phase20_PreBetaManualChecks_Runbook.md` owns the manual handoff. `PDV_AllRaceDaedricBetaReadinessLedger.md` is the current beta-feel blocker ledger: Daedric D-15..D-18 are locked, all sixteen Skyrim-present Prince rows are drafted, and Batch 0 static D-18 proof is complete; remaining blockers are per-Prince CAT-6 promotion/readback, runtime or display proof, and stack/Survey legibility.
...
- **Content-authoring ratification audit + stale Daedric-gate fix (2026-05-31 AEST):** A whole-roster gap audit confirmed the V1 manifest prose is content-complete: `pdv_content_verify` clean (FAIL=0, WARN=0, 1,065 rows), all 10 races and all 16 Skyrim-present Princes carry their full V1 surface set, no required V1 surface missing, and guardrail spot-checks passed. The 1.0 Acceptance Tracker "Content authored" column is flipped from `Drafted` to `Ratified` for all 10 races (new status value defined), and coverage matrix 20B/20C carry `contentAuthoringStatus` notes. Remaining content work is out of scope: CAT-6 ESP promotion (code track) and V2 voiced dialogue. **Reconciliation of a Codex "all missing" report:** the strict Phase 20 verifier read `references/authoring/PDV_Phase20_NoInGameProof_Gates.json` `daedricBlockers`, which still marked stigma/curse-access/Prince-order as `open` (stale - D-15/D-16/D-17 resolved them this session) and `tools/pdv_verify.mjs` line ~3734 asserted they must be `open` to pass, so it reported Daedric as blocked. Fixed both: the gate JSON now marks the three contracts `resolved-by-D15/D16/D17` (keeping `runtimePromotionAllowed: false`, since promotion is still gated by CAT-6 + the per-Prince effect review), and the verifier now passes when decisions are resolved and promotion stays gated. The genuine remaining "missing" is ESP target records (CAT-6, not done) and planned `PDV_REFR_*`/`PDV_DIAL_*` placement/recognition records (code track / V2), not missing prose.
