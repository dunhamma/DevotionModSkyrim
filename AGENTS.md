...
| `tools/pdv_quest_reaction_build.mjs` | Deterministic V3 Quest Reaction catalog compiler/checker | Generating or exact-byte-checking the two fully-qualified v2 catalogs and SHA-256 receipt from the compatibility manifest plus canonical CSV inputs; backend proof only until the combined fresh-game smoke |
...
| `tools/pdv_quest_reaction_runtime_check.mjs` | Read-only quest-reaction queue log checker | Checking QR_QUEUE lifecycle, FIFO/coalescing/completion sequence, cross-session-safe START-to-COMPLETE latency despite reused `v3qr_N` IDs, ingress-build timing, overflow, stack-dump/broad-scope-abort markers, and normal save-load freeze/thaw observations after an in-game sweep |
