...
- **Phase 20 no-in-game-proof workplan (2026-05-31 AEST):** `references/authoring/PDV_Phase20_NoInGameProof_Workplan.md` now defines the rest of Phase 20 work that can continue before additional Skyrim runtime proof: gate-ledger hardening, normal-play hook contracts, static verifier expansion, final-world placement contracts, stack/ceiling audits, Survey/status copy prep, CAT-6 target-record prep, recognition packet prep, and Daedric blocker closeout. `references/authoring/PDV_Phase20_NoInGameProof_Gates.json` is the structured gate packet now checked by `tools/pdv_verify.mjs --strict-phase20-race-costing`: all ten races stay `Fail - runtime/manual proof deferred`, P0/P1 lanes have final-placement contracts, P2 lanes remain audit-only, CAT-6 remains target-record-needed, recognition remains CK-packet-prep only, and Daedric runtime promotion remains blocked on stigma, curse-access template, and Prince order. `references/authoring/PDV_Phase20_ManualEvidenceLedger.json` is the matching pending evidence intake, also checked by the strict gate so future manual proof has slots without silently becoming a pass claim. The lane explicitly forbids marking race `Pass`, claiming final placement, or broad-promoting rewards/strings without manual/runtime evidence.
