...
| `tools/pdv_copy_census.mjs` | Read-only player-facing copy census and UX-workbench generator | Keeping live runtime text, current gameplay contracts, and prior writing references independently traceable before prose edits; `--refresh-live` reads `Devotion.esp` through direct houseCARL batches, while the census, Nord/Kyne packet, race-batched formal-offer view, dynamic-piety viability model, full Penpot-importable journey map, stable copy-to-flow assignments, UX surface catalogue, editable Penpot surface library, and dimension-aware popup writing frames remain ignored and regenerable under `generated/pdv-copy-census/`. Normal player flow includes explicit origin-choice, curse-transition, cultural-rite, lunar-focus, reckoning, and system/fallback families; `Debug*` Papyrus functions remain excluded. |
| `tools/pdv_copy_roundtrip.mjs` | Owner-prose exchange exporter and review-plan importer | Exporting the current census as an Excel-friendly stable-ID CSV, then validating owner-edited status/draft/note cells into a no-write review plan. It never edits Papyrus, Prisma, data, or `Devotion.esp`; ESP rows remain explicitly `housecarl-required`. |
| `references/authoring/PDV_UXSurfaceCatalogue.json` | UX surface planning authority | Defining stable, reusable Skyrim/PDV surface templates, constraints, dependencies, cautions, fallbacks, and implementation routes for the generated catalogue and Penpot library. Selecting or annotating a template is design evidence only; it does not authorize a dependency or game change. |
| `references/authoring/PDV_RaceArchitectureAtlas.json` | Race architecture and implementation-audit authority (shared Claude/Codex) | Reading or updating the high-level per-race architecture map: all 10 races across Spine / Substrate / Curse lanes, the shared Daedric track, and the cross-race matrix. Each node has a stable `nodeId`, design `status`, independent Papyrus/ESP/reachability/copy evidence, and a runtime-proof boundary. The compact `implementationAudit.queue` holds actionable discrepancies only; exact prose remains in the workbook. Edit HERE -- the HTML and audit projections are generated. |
| `tools/pdv_atlas_render.mjs` | Race atlas renderer and data gate | Rendering the atlas JSON to standalone HTML (`node tools/pdv_atlas_render.mjs`, default output `generated/PDV_RaceArchitectureAtlas.html`, gitignored). `--check` validates statuses, stable node IDs, audit verdicts, queue references, and matrix width without rendering. Never commit or hand-edit the HTML. |
| `tools/pdv_implementation_audit_export.mjs` | Race implementation-audit projection gate | Validating atlas queue/node/copy links against the exact 5,166-row census and workbook exchange, then writing ignored JSON plus compact queue/copy-link CSVs under `generated/pdv-race-implementation-audit/`. It never reads or writes an ESP and never changes owner prose; ESP evidence must first come from direct houseCARL readback recorded in the atlas. |
...
- **[2026-08-21] - Copy uplift and refined KID filtering live on a 2.0 descendant, with owner prose kept as data:** `fix/2.0-copy-uplift` is rooted at the current `feature/v3-big-update` integration tip. It carries the narrower SunHelm water exclusion and the already-disabled Hircine/Zenithar KID lanes, plus the read-only copy census and Nord/Kyne workbench. Approved commitment headings/bodies live in versioned `PDV_CommitmentOfferCopy.json`; 2.0 translates the choice state into `PDV_DevotionLedger` and the refusal/deferral action into `PDV_DebugRuntime` rather than restoring those bodies to the manager. The copy workbench now separates the Penpot-owned full journey/adjacency map from an Excel-friendly owner-prose exchange joined by stable copy IDs; import validates protected census context and produces a review-only routing plan, never source or ESP writes. The journey map explicitly classifies origin choices, curse transitions, cultural rites, Khajiit lunar focus, pact reckonings, and system/fallback UX; all normal live rows in the current census snapshot have a flow assignment, while Papyrus `Debug*` functions are excluded as developer surfaces. Repository-source compilation passed for Ledger, DebugRuntime, and Manager with zero warnings; census/round-trip tests, deterministic regeneration, and the formal-offer source gate passed. This is static/build proof only. The new `PDV_GLO_CommitmentNotYetAvailable` ledger property still needs direct plugin wiring/readback, followed by live-source compilation and in-game presentation/save-load proof. Durable rule: when working copy changes cross the 2.0 extraction boundary, replay behavior into the owning deep module instead of copying an older manager body.
- **[2026-08-21] - UX surface exploration is a regenerable design layer, not implementation authority:** The copy workbench now generates a 20-template Skyrim/PDV surface catalogue, an editable Penpot SVG library, and five dimension-aware popup writing frames from `PDV_UXSurfaceCatalogue.json`. Templates cover established Skyrim surfaces, PDV's existing stack, optional framework-backed surfaces, and bespoke UI, with stable IDs, dependency/cost constraints, cautions, fallbacks, and implementation routes. Prisma frame dimensions are measured from production CSS; vanilla MessageBox sizing is explicitly a conservative design target because resolution, font, and UI replacers vary. Character and visible-line figures are editorial budgets, not engine truncation claims. Owner decisions remain editable in the Excel-readable catalogues while exact authoritative prose remains in the prose exchange. The `flow.unresolved` node remains a fail-closed classifier but is materialized on review boards only when at least one row is actually unresolved; a zero-count warning must not make completed classification look open. Choosing or modifying a template does not install a dependency, edit the ESP, or authorize wire-in; those decisions require a later reviewed implementation tranche.
- **[2026-08-24] - Historical copy disposition is separate from live prose and implementation approval:** The accessible prose workbook now carries a row-level `Disposition Audit` for all 493 unmatched May-manifest rows while preserving the 4,173 live player-facing rows and the exact 5,166-row round-trip schema. The audit classifies 311 as reconciled to renamed/shared/dynamic current surfaces, 48 as stale proposals, 87 as historical or explicitly future material (including all 29 V2 recognition-dialogue rows), 45 as optional new UX implementation candidates, and 2 as manual design contradictions. These are findings, not automatic deletes or feature approvals. Exact prose remains owner-controlled in the writing sheets; Penpot remains design evidence; only an explicitly approved cross-linked implementation packet may change game authority. The Claude UX continuation contract is `handoff/PDV_UX_Claude_Handoff_2026-08-24.md`.
- **[2026-08-24] - Race UX audit separates record presence from source-version and runtime proof:** The Race Architecture Atlas now assigns stable IDs and separate Papyrus, ESP, reachability, copy, and runtime-proof evidence to its 132 race moments plus eight shared Daedric nodes. Direct houseCARL readback confirmed the Imperial offers/responses, Concordat track, substrate, civic rewards, runtime quest, extracted service bindings, and ten-member origin-adapter FormList. The active `Devotion-V3Dev` tree and repo mirror both contain the extracted architecture, but only 92 PDV scripts are byte-identical, 26 differ, and `PDV_TestRunbook.psc` is active-tree-only; this is one shared source-version blocker for atlas route claims, not ninety independent race defects. Direct PEX review also confirmed that Manager `OriginRuntime` is intentionally runtime-filled, the Imperial Akatosh `HealSpell` has a coded fallback, and the Concordat extreme-state array is inactive under its false gate, so none became a false UX backlog item. No runtime claim follows from record presence. The global copy refresh captures only the first 500 of 515 Message records; all ten race-scoped Message families reconcile exactly, but the unpaged remainder remains an audit-tooling gap. The visible implementation queue therefore contains only three Imperial UX questions (curse communication, Champion entry, Concordat state beats), keeps technical blockers separate, and preserves all exact prose/reference rows by stable copy ID.
