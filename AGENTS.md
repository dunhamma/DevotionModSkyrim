...
| `references/authoring/PDV_Phase20_NoInGameProof_Gates.json` | Structured Phase 20 no-in-game gate packet | Machine-checking all ten race no-game statuses, hook contracts, final-placement contracts, stack snapshots, CAT-6 target-record state, recognition packet prep, and Daedric blocker state |
...
| `references/authoring/PDV_RecognitionDialogueScalePacket.md` | CK-safe recognition/dialogue scale packet | Keeping recognition lines curated and CK-authored, with manifest/readback/runtime proof before scaling beyond the current Nord dialogue set |
...
- **Architecture packets before next grind (2026-05-31 AEST):** Three risk packets now close the "architect before grinding content" gap. `PDV_PreBetaRaceAcceptanceRubric.md` defines pass/conditional/fail criteria for race scaling, `PDV_RecognitionDialogueScalePacket.md` defines the CK-safe recognition/dialogue pattern and keeps generated dialogue out of scope, and `PDV_CAT6PromotionPilot.md` defines the first low-risk CAT-6 draft-to-ESP-to-handbook promotion proof. The next Altmer scaling pass should use the rubric; broad recognition/dialogue and broad string promotion should wait for their packet gates.
